...
public class ActionDispatch {
	...
	public static DynamicDict dispatch(DynamicDict dict) throws Exception {
		...
		ConnectionContextUtil.createConnectionContext();// 创建数据库连接上下文 add by
														// easonwu 2009-12-22
	...
	/**
	 * 支持多个数据库连接的处理 BHO调用 如果是XA,则采用JTA方式，否则通过直接的commit和rollback来操作
	 * 
	 * @param dto
	 * @param strDBName
	 ...
	 * @return
	 */
	private static DynamicDict performDictMulti(DynamicDict dto)
			throws Exception {
		boolean commit = true;
		...
		try {
			...
			FrameServiceController controller = FrameServiceController
					.getInstance();
