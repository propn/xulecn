...
public class SpOutMsgSynhbDAO extends AbstractDAO {
	
	//查询SQL
	private String SQL_SELECT_ORACLE = "select t.id,t.in_time,t.order_id,t.oiut_order_id,t.out_stream_no,t.order_result,t.feeback_result,t.state,t.feeback_desc,t.feeback_time,t.int_sys_id,(select s.name from sp_int_sys s where int_sys_id= s.int_sys_id and rownum = 1) int_sys_name,t.thread_id,t.prod_no from SP_OUT_MSG_SYNHB t where 1=1 ";
	
//	查询SQL
	private String SQL_SELECT_INFORMIX = "select t.id,t.in_time,t.order_id,t.oiut_order_id,t.out_stream_no,t.order_result,t.feeback_result,t.state,t.feeback_desc,t.feeback_time,t.int_sys_id,(select min(s.name) from sp_int_sys s where int_sys_id= s.int_sys_id ) int_sys_name,t.thread_id,t.prod_no from SP_OUT_MSG_SYNHB t where 1=1 ";
	

	//	统计总数SQL 
	private String SQL_SELECT_COUNT = "select count(*) as col_counts from sp_out_msg_synhb t where 1=1 ";

	//	insert SQl
	private String SQL_INSERT = "insert into SP_OUT_MSG_SYNHB (id, in_time, order_id, oiut_order_id, out_stream_no, order_result, msg, feeback_result, feeback_msg, state, feeback_desc, feeback_time, int_sys_id, thread_id) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

	//	普通update SQL
	private String SQL_UPDATE = "update SP_OUT_MSG_SYNHB set id=?, in_time=?, order_id=?, oiut_order_id=?, out_stream_no=?, order_result=?, msg=?, feeback_result=?, feeback_msg=?, state=?, feeback_desc=?, feeback_time=?, int_sys_id=?, thread_id=? where 1=1 ";
	
	//	普通delete SQL
	private String SQL_DELETE = "delete from SP_OUT_MSG_SYNHB where 1=1 ";
	
		//	根据主键delete SQL
	private String SQL_DELETE_KEY = "delete from SP_OUT_MSG_SYNHB where id=?";
		
		//	根据主键update SQL
	private String SQL_UPDATE_KEY = "update SP_OUT_MSG_SYNHB set id=?, in_time=?, order_id=?, oiut_order_id=?, out_stream_no=?, order_result=?, msg=?, feeback_result=?, feeback_msg=?, state=?, feeback_desc=?, feeback_time=?, int_sys_id=?, thread_id=? where id=?";
		
		//	根据主键查询SQL
	private String SQL_SELECT_KEY_ORACLE = "select id,in_time,order_id,oiut_order_id,out_stream_no,order_result,msg,feeback_result,feeback_msg,state,feeback_desc,feeback_time,int_sys_id,(select s.name from sp_int_sys s where int_sys_id= s.int_sys_id and rownum = 1) int_sys_name,thread_id,prod_no from SP_OUT_MSG_SYNHB where id=? ";
	
	private String SQL_SELECT_KEY_INFORMIX = "select id,in_time,order_id,oiut_order_id,out_stream_no,order_result,msg,feeback_result,feeback_msg,state,feeback_desc,feeback_time,int_sys_id,(select min(s.name) from sp_int_sys s where int_sys_id= s.int_sys_id ) int_sys_name,thread_id,prod_no from SP_OUT_MSG_SYNHB where id=? ";
	//	当前DAO 所属数据库name
	private String dbName = JNDINames.DEFAULT_DATASOURCE ;


	public SpOutMsgSynhbDAO() {
