...
public class PrivilegeService extends DictService {
	 ...
	 * 
	 * @param privCode
	 *            权限条件编码（VARCHAR(50)）：根据编码类型传对应的值，例如当编码类型为0时，传权限ID；
	 * @param regionId
	 * @param regionType
	 *            管理区域类型 region_type '0':计费线的地域 '1':资源线地域 '2':营销的地域 '3':营销线的组织
	 * @param privType
	 *            权限条件类型：0-权限ID，1-权限编码，2-菜单ID，3-菜单编码；
	 * @return
	 * @throws Exception
	 */
	public boolean isPrivilegeInRegion(String privCode, String regionId,
			String regionType, String privType) throws Exception {
		...
		for (int i = 0; i < privCodeList.size(); i++) {
			for (int j = 0; j < roleState.length; j++) {
				...
				if (pCode.equals(roleState[j].getPrivilegeCode())
						&& regionType.equals(roleState[j].getPermitionFlag())) {
	 ...
	 * @throws Exception
	 */
	private ArrayList getCustCtrlDataInfoFromDatabase(String pageCode,
			String datasetCode, String custTypeId) throws Exception {
		ArrayList privilegeIds = this.getStaffPrivilegeId("", "99I");
		if (privilegeIds.size() == 0) {
			return new ArrayList();
		}

		DynamicDict dto = getServiceDTO("PrivilegeBO",
	 ...
	 * @throws Exception
	 */
	public ArrayList getCustCtrlDataInfo(String pageCode, String datasetCode,
			String custTypeId) throws Exception {
		ArrayList custCtrlDataList = (ArrayList) RequestContext.getContext()
				.getHttpSession().getAttribute("CustCtrlDataList");
		ArrayList returnList = new ArrayList();
		if (custCtrlDataList != null) {// 如果Session中已经有缓存
			for (int i = 0; i < custCtrlDataList.size(); i++) {
				CustCtrlInfoVo custCtrlInfoVo = (CustCtrlInfoVo) custCtrlDataList
						.get(i);
		...
		if (custCtrlDataList == null || returnList.size() == 0) {
			...
			ArrayList queryDataList = getCustCtrlDataInfoFromDatabase(pageCode,
					datasetCode, custTypeId);
			if (queryDataList.size() == 0) {// 如果查询数据库也查询不到数据
				...
				RequestContext.getContext().getHttpSession()
						.setAttribute("CustCtrlDataList", custCtrlDataList);
				return new ArrayList();
				...
				if (custCtrlDataList == null) {
					...
					RequestContext.getContext().getHttpSession()
							.setAttribute("CustCtrlDataList", queryDataList);
				} else {
	 ...
	 * 
	 * @param partyRoleId
	 *            预留参数,程序不会使用这个传递进来的参与人角色ID,而是从Session中 获取当前登陆员工的参与人角色ID
	 * @return
	 * @throws Exception
	 */
	public ArrayList getPricingParamCatalogByPrivilege(String partyRoleId)
			throws Exception {
		...
		DynamicDict dto = getServiceDTO("PrivilegeBO",
				"getPricingParamCatalogByPrivilege");
	 ...
	 * @throws Exception
	 */
	public ArrayList getOrganizationByPrivilege(String partyRoleId,
			String cProductId, HttpServletRequest request) throws Exception {
		ArrayList privilegeIds = this.getStaffPrivilegeIdFromSession(
				partyRoleId, "99X", request);// 查询员工拥有的类型权限
		if (privilegeIds.size() == 0) {// 如果没有权限,返回空列表
			return new ArrayList();
		}

		DynamicDict dto = getServiceDTO("PrivilegeBO",
				"getOrganizationByPrivilege");
		dto.setValueByName("parameter", partyRoleId);
		dto = ActionDispatch.dispatch(dto);
		return ((ArrayList) dto.getValueByName("result"));
	}
	 ...
	 * @throws Exception
	 */
	public ArrayList getOrgForNoHeadByPrivilege(String partyRoleId,
			String noFamilyId, HttpServletRequest request) throws Exception {
		ArrayList privilegeIds = this.getStaffPrivilegeIdFromSession(
				partyRoleId, "99Y", request);// 查询员工拥有的类型权限
		if (privilegeIds.size() == 0) {// 如果没有权限,返回空列表
			return new ArrayList();
		}
	 ...
	 * @throws Exception
	 */
	public ArrayList getOrgForDeliverNoByPrivilege(String partyRoleId,
			String noFamilyId, HttpServletRequest request) throws Exception {

		ArrayList privilegeIds = this.getStaffPrivilegeIdFromSession(
				partyRoleId, "99Z", request);// 查询员工拥有的类型权限
		if (privilegeIds.size() == 0) {// 如果没有权限,返回空列表
			return new ArrayList();
		}

		DynamicDict dto = getServiceDTO("PrivilegeBO",
				"getOrgForDeliverNoByPrivilege");
		Map m = new HashMap();
		m.put("partyRoleId", partyRoleId);
		m.put("noFamilyId", noFamilyId);
		dto.setValueByName("parameter", m);
	 ...
	 * 
	 * @param partyRoleId
	 *            员工ID,预留参数,程序不会使用这个传递进来的参与人角色ID,而是从 Session中获取当前登陆员工的参与人角色ID
	 * @param offerId
	 *            商品ID
	 */
	public ArrayList getOfferServiceByPrivilege(String partyRoleId,
	 ...
	 *            员工标识(预留)
	 * @param privilegeType
	 *            权限使用类型: 99B 叶子菜单项 99F 商品服务提供 99G 定价参数目录 99H 号码等级 99I 客户类型 99J
	 *            综合查询
	 * @return
	 */
	public ArrayList getStaffPrivilegeIdFromSession(String partyRoleId,
			String privilegeType, HttpServletRequest request) {
		// 从Session中查找从鉴权服务器返回的用户登陆信息
		return getStaffPrivilegeIdFromSession(partyRoleId, privilegeType,
				request.getSession());
	}
	 ...
	 */
	public ArrayList getStaffPrivilegeIdFromSession(String partyRoleId,
			String privilegeType, HttpSession session) {
		// 从Session中查找从鉴权服务器返回的用户登陆信息
		LoginRespond respond = (LoginRespond) session
	 ...
	 * @return
	 */
	public ArrayList getStaffPrivilegeId(String partyRoleId,
			String privilegeType) {
		return this.getStaffPrivilegeIdFromSession(partyRoleId, privilegeType,
	...
	public boolean checkPrivilegeCode(String privilegeCode) throws Exception {
		return checkPrivilegeCodeFromSession(RequestContext.getContext()
				.getHttpRequest(), privilegeCode);
	}

	public boolean checkPrivilegeCodeFromSession(HttpServletRequest req,
			String privilegeCode) throws Exception {
	...
	/**
	 * 这个服务函数用于校验客户端传上来的权限是否为当前用户所拥有的权限, 将当前用户所拥有 的权限过滤掉,返回当前用户所没有的权限.
	 * 
	...
	/**
	 * 这个服务函数用于校验客户端传上来的权限是否为当前用户所拥有的权限, 将当前用户所拥有的 权限过滤掉,返回当前用户所没有的权限.
	 * 
	 * @param privilegesToCheck
	 * @param session
	 * @return
	 ...
	 * 
	 * @return 操作结果, 成功时ServiceResult.resultObject为适用于TreeList组件的所有菜单构成的XML 字符串
	 */
	...
	public ArrayList getDataPrivilegeByPrivilegeType(String privType,
			String privId) throws Exception {
		DynamicDict dto = getServiceDTO("PrivilegeBO",
				"getDataPrivilegeByPrivilegeType");
		Map m = new HashMap();
		m.put("privType", privType);
		m.put("privId", privId);
		dto.setValueByName("parameter", m);
	...
	public ArrayList getDataPrivilegeByPrivilegeId(String privilegeId,
			String type) throws Exception {

		DynamicDict dto = getServiceDTO("PrivilegeBO",
				"getDataPrivilegeByPrivilegeId");
		Map m = new HashMap();
	...
	public void addDataPrivileges(MmDataPrivVO[] dataPrivileges, String privType)
			throws Exception {
	...
	public void addMenuDataPrivileges(ArrayList dataPrivileges)
			throws Exception {
	...
	public String getPrivilegeListWithRegionRelatByName(String privilegeName)
			throws Exception {
		LoginRespond loginRespond = (LoginRespond) RequestContext.getContext()
				.getHttpSession().getAttribute("LoginRespond");
		RoleState roleStates[] = loginRespond.getRoleState();
		...
		DynamicDict dto = getServiceDTO("PrivilegeBO",
				"getPrivilegeListWithRegionRelatByName");
		Map m = new HashMap();
	...
	public String getPrivilegeListWithRegionRelatByParentId(String parentId)
			throws Exception {
		LoginRespond loginRespond = (LoginRespond) RequestContext.getContext()
				.getHttpSession().getAttribute("LoginRespond");
		RoleState roleStates[] = loginRespond.getRoleState();
		...
		DynamicDict dto = getServiceDTO("PrivilegeBO",
				"getPrivilegeListWithRegionRelatByParentId");
		Map m = new HashMap();
	 ...
	 *         成功时ServiceResult.resultObject为指定角色对应的所有权限构成的适用于TreeList组件的xml字符串
	 */
	public ArrayList getPrivByRoleId(String roleId, String privilegeType)
			throws Exception {
	...
	public List getPrivilegeVosByPartyRoleId(String partyRoleId)
			throws Exception {

		DynamicDict dto = getServiceDTO("PrivilegeBO",
				"getPrivilegeVosByPartyRoleId");
		dto.setValueByName("parameter", partyRoleId);
		dto = ActionDispatch.dispatch(dto);
	...
	// 查询工号所拥有的权限Id
	public ArrayList getPrivilegeIdsByPartyRoleId(String partyRoleId)
			throws Exception {

		DynamicDict dto = getServiceDTO("PrivilegeBO",
				"getPrivilegeIdsByPartyRoleId");
		dto.setValueByName("parameter", partyRoleId);
		dto = ActionDispatch.dispatch(dto);
		return ((ArrayList) dto.getValueByName("result"));
