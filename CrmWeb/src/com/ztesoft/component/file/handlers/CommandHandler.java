...
public class CommandHandler {
	...
	private String name;
	private static Map getCommands = new HashMap(
	        3);
	private static Map postCommands = new HashMap(
	        2);
	...
	public static CommandHandler valueOf(final String name) throws IllegalArgumentException {
		...
		if (!isValidForGet(name) && !isValidForPost(name))
			throw new IllegalArgumentException();
		return (getCommands.get(name) != null) ? (CommandHandler)getCommands.get(name) : (CommandHandler)postCommands.get(name);
	}
