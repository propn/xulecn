...
//	public static void setInterTime(int pInterTime){
//		interTime=pInterTime;//初始化线程扫描间隔
//		if(servicePool!=null){
//			for (int i=0;i<servicePool.length;i++)
//			{
//				ThreadRun _t=servicePool[i];
//				if(_t!=null)
//				_t.setInterTime(pInterTime);
//			}
//		}
//	}
...
//	}
	...
	{
//		synchronized(syncObject){//如果同时改,那么需要等待
//			syncObject.notifyAll();//每次执行都唤醒全部进程
			int i=getThreadIdx();//获取一个可以执行的现成线程索引
//			System.out.println("获取的线程池索引是:"+i+"\n");
			try
			{
				if (i!=-1)
				{
