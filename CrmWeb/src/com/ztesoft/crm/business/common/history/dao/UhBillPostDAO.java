...
public class UhBillPostDAO extends AbstractDAO {
	
	//查询SQL
	private String SQL_SELECT = "select ord_item_id,cust_ord_id,ord_id,action_type,state_date,end_time,beg_time,serv_id,bill_type,owner_type,post_method,post_code,incept_person,post_address,email,contact _name,contact_phone,guard_post,certificate_type,certificate_no,query_password from UH_BILL_POST where 1=1 ";
	
	//	统计总数SQL
	private String SQL_SELECT_COUNT = "select count(*) as col_counts from uh_bill_post where 1=1 ";

	//	insert SQl
	private String SQL_INSERT = "insert into UH_BILL_POST (ord_item_id, cust_ord_id, ord_id, action_type, state_date, end_time, beg_time, serv_id, bill_type, owner_type, post_method, post_code, incept_person, post_address, email, contact _name, contact_phone, guard_post, certificate_type, certificate_no, query_password) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

	//	普通update SQL
	private String SQL_UPDATE = "update UH_BILL_POST set ord_item_id=?, cust_ord_id=?, ord_id=?, action_type=?, state_date=?, end_time=?, beg_time=?, serv_id=?, bill_type=?, owner_type=?, post_method=?, post_code=?, incept_person=?, post_address=?, email=?, contact _name=?, contact_phone=?, guard_post=?, certificate_type=?, certificate_no=?, query_password=? where 1=1 ";
	
	//	普通delete SQL
	private String SQL_DELETE = "delete from UH_BILL_POST where 1=1 ";
	
		//	根据主键delete SQL
	private String SQL_DELETE_KEY = "delete from UH_BILL_POST where ord_item_id=?";
		
		//	根据主键update SQL
	private String SQL_UPDATE_KEY = "update UH_BILL_POST set ord_item_id=?, cust_ord_id=?, ord_id=?, action_type=?, state_date=?, end_time=?, beg_time=?, serv_id=?, bill_type=?, owner_type=?, post_method=?, post_code=?, incept_person=?, post_address=?, email=?, contact _name=?, contact_phone=?, guard_post=?, certificate_type=?, certificate_no=?, query_password=? where ord_item_id=?";
		
		//	根据主键查询SQL
	private String SQL_SELECT_KEY = "select ord_item_id,cust_ord_id,ord_id,action_type,state_date,end_time,beg_time,serv_id,bill_type,owner_type,post_method,post_code,incept_person,post_address,email,contact _name,contact_phone,guard_post,certificate_type,certificate_no,query_password from UH_BILL_POST where ord_item_id=? ";
	
	//	当前DAO 所属数据库name
	private String dbName = "ORD" ;


	public UhBillPostDAO() {
	...
	public List translateInsertParams(Map map) throws FrameException{
		...
		params.add(map.get("contact _name")) ;
	...
	public List translateUpdateParams(Map vo , List condParas) throws FrameException{
		...
		params.add(vo.get("contact _name")) ;
