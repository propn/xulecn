...
public class ArcProductOffDetaDAO extends AbstractDAO {
	
	//查询SQL
	private String SQL_SELECT = "select offer_detail_id,seq,offer_id,comp_role_id,role_min_num,role_max_num,element_type,include_flag,element_id,element_desc,time_limit_type,end_time,object_amount_start,object_amount_end,rela_type,join_max_count,pricing_object_id,send_bill,group_opt,state,order_id,party_id,party_role_id,oper_region_id,oper_date,ord_action_type,ord_no,cancel_ord_no,event_seq,default_flag,cancel_event_seq from ARC_PRODUCT_OFFER_DETAIL where 1=1 ";
	
	//	统计总数SQL
	private String SQL_SELECT_COUNT = "select count(*) as col_counts from arc_product_offer_detail where 1=1 ";

	//	insert SQl
	private String SQL_INSERT = "insert into ARC_PRODUCT_OFFER_DETAIL (offer_detail_id, seq, offer_id, comp_role_id, role_min_num, role_max_num, element_type, include_flag, element_id, element_desc, time_limit_type, end_time, object_amount_start, object_amount_end, rela_type, join_max_count, pricing_object_id, send_bill, group_opt, state, order_id, party_id, party_role_id, oper_region_id, oper_date, ord_action_type, ord_no, cancel_ord_no, event_seq, default_flag, cancel_event_seq) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

	//	普通update SQL
	private String SQL_UPDATE = "update ARC_PRODUCT_OFFER_DETAIL set offer_detail_id=?, seq=?, offer_id=?, comp_role_id=?, role_min_num=?, role_max_num=?, element_type=?, include_flag=?, element_id=?, element_desc=?, time_limit_type=?, end_time=?, object_amount_start=?, object_amount_end=?, rela_type=?, join_max_count=?, pricing_object_id=?, send_bill=?, group_opt=?, state=?, order_id=?, party_id=?, party_role_id=?, oper_region_id=?, oper_date=?, ord_action_type=?, ord_no=?, cancel_ord_no=?, event_seq=?, default_flag=?, cancel_event_seq=? where 1=1 ";
	
	//	普通delete SQL
	private String SQL_DELETE = "delete from ARC_PRODUCT_OFFER_DETAIL where 1=1 ";
	
		//	根据主键delete SQL
	private String SQL_DELETE_KEY = "delete from ARC_PRODUCT_OFFER_DETAIL where offer_detail_id=? and seq=?";
		
		//	根据主键update SQL
	private String SQL_UPDATE_KEY = "update ARC_PRODUCT_OFFER_DETAIL set offer_detail_id=?, seq=?, offer_id=?, comp_role_id=?, role_min_num=?, role_max_num=?, element_type=?, include_flag=?, element_id=?, element_desc=?, time_limit_type=?, end_time=?, object_amount_start=?, object_amount_end=?, rela_type=?, join_max_count=?, pricing_object_id=?, send_bill=?, group_opt=?, state=?, order_id=?, party_id=?, party_role_id=?, oper_region_id=?, oper_date=?, ord_action_type=?, ord_no=?, cancel_ord_no=?, event_seq=?, default_flag=?, cancel_event_seq=? where offer_detail_id=? and seq=?";
		
		//	根据主键查询SQL
	private String SQL_SELECT_KEY = "select offer_detail_id,seq,offer_id,comp_role_id,role_min_num,role_max_num,element_type,include_flag,element_id,element_desc,time_limit_type,end_time,object_amount_start,object_amount_end,rela_type,join_max_count,pricing_object_id,send_bill,group_opt,state,order_id,party_id,party_role_id,oper_region_id,oper_date,ord_action_type,ord_no,cancel_ord_no,event_seq,default_flag,cancel_event_seq from ARC_PRODUCT_OFFER_DETAIL where offer_detail_id=? and seq=? ";
	
	//	当前DAO 所属数据库name
	private String dbName = JNDINames.PM_DATASOURCE ;


	public ArcProductOffDetaDAO() {
