...
public class ArcProductOffAttrDAO extends AbstractDAO {
	
	//查询SQL
	private String SQL_SELECT = "select offer_attr_seq,seq,offer_id,offer_attr_value,field_name,allow_customized_flag,excluablity,element_type,element_id,attr_value_type_id,attr_value_unit_id,offer_attr_character,if_default_value,default_value,order_id,attr_length,is_null,action_id,is_check,is_make_to_table,is_edit,is_make,check_message,attr_format,colspan,page_url,page_field_name,page_field,make_field,param_field,param_field_name,is_send_bill,is_rela_price,is_print,state,oper_date,ord_action_type,ord_no,cancel_ord_no,event_seq,cancel_event_seq,party_id,party_role_id,oper_region_id from ARC_PRODUCT_OFFER_ATTR where 1=1 ";
	
	//	统计总数SQL
	private String SQL_SELECT_COUNT = "select count(*) as col_counts from arc_product_offer_attr where 1=1 ";

	//	insert SQl
	private String SQL_INSERT = "insert into ARC_PRODUCT_OFFER_ATTR (offer_attr_seq, seq, offer_id, offer_attr_value, field_name, allow_customized_flag, excluablity, element_type, element_id, attr_value_type_id, attr_value_unit_id, offer_attr_character, if_default_value, default_value, order_id, attr_length, is_null, action_id, is_check, is_make_to_table, is_edit, is_make, check_message, attr_format, colspan, page_url, page_field_name, page_field, make_field, param_field, param_field_name, is_send_bill, is_rela_price, is_print, state, oper_date, ord_action_type, ord_no, cancel_ord_no, event_seq, cancel_event_seq, party_id, party_role_id, oper_region_id) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

	//	普通update SQL
	private String SQL_UPDATE = "update ARC_PRODUCT_OFFER_ATTR set offer_attr_seq=?, seq=?, offer_id=?, offer_attr_value=?, field_name=?, allow_customized_flag=?, excluablity=?, element_type=?, element_id=?, attr_value_type_id=?, attr_value_unit_id=?, offer_attr_character=?, if_default_value=?, default_value=?, order_id=?, attr_length=?, is_null=?, action_id=?, is_check=?, is_make_to_table=?, is_edit=?, is_make=?, check_message=?, attr_format=?, colspan=?, page_url=?, page_field_name=?, page_field=?, make_field=?, param_field=?, param_field_name=?, is_send_bill=?, is_rela_price=?, is_print=?, state=?, oper_date=?, ord_action_type=?, ord_no=?, cancel_ord_no=?, event_seq=?, cancel_event_seq=?, party_id=?, party_role_id=?, oper_region_id=? where 1=1 ";
	
	//	普通delete SQL
	private String SQL_DELETE = "delete from ARC_PRODUCT_OFFER_ATTR where 1=1 ";
	
		//	根据主键delete SQL
	private String SQL_DELETE_KEY = "delete from ARC_PRODUCT_OFFER_ATTR where offer_attr_seq=? and seq=? and offer_id=?";
		
		//	根据主键update SQL
	private String SQL_UPDATE_KEY = "update ARC_PRODUCT_OFFER_ATTR set offer_attr_seq=?, seq=?, offer_id=?, offer_attr_value=?, field_name=?, allow_customized_flag=?, excluablity=?, element_type=?, element_id=?, attr_value_type_id=?, attr_value_unit_id=?, offer_attr_character=?, if_default_value=?, default_value=?, order_id=?, attr_length=?, is_null=?, action_id=?, is_check=?, is_make_to_table=?, is_edit=?, is_make=?, check_message=?, attr_format=?, colspan=?, page_url=?, page_field_name=?, page_field=?, make_field=?, param_field=?, param_field_name=?, is_send_bill=?, is_rela_price=?, is_print=?, state=?, oper_date=?, ord_action_type=?, ord_no=?, cancel_ord_no=?, event_seq=?, cancel_event_seq=?, party_id=?, party_role_id=?, oper_region_id=? where offer_attr_seq=? and seq=? and offer_id=?";
		
		//	根据主键查询SQL
	private String SQL_SELECT_KEY = "select offer_attr_seq,seq,offer_id,offer_attr_value,field_name,allow_customized_flag,excluablity,element_type,element_id,attr_value_type_id,attr_value_unit_id,offer_attr_character,if_default_value,default_value,order_id,attr_length,is_null,action_id,is_check,is_make_to_table,is_edit,is_make,check_message,attr_format,colspan,page_url,page_field_name,page_field,make_field,param_field,param_field_name,is_send_bill,is_rela_price,is_print,state,oper_date,ord_action_type,ord_no,cancel_ord_no,event_seq,cancel_event_seq,party_id,party_role_id,oper_region_id from ARC_PRODUCT_OFFER_ATTR where offer_attr_seq=? and seq=? and offer_id=? ";
	
	//	当前DAO 所属数据库name
	private String dbName = JNDINames.PM_DATASOURCE ;


	public ArcProductOffAttrDAO() {
