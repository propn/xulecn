...
public class ArcOffRelationRestrictedDAO extends AbstractDAO {
	
	//查询SQL
	private String SQL_SELECT = "select offer_restricted_id,relation_id,product_offer_id,role_type,restricted_type,restricted_product_id,state,state_date,seq,party_id,party_role_id,oper_region_id,oper_date,ord_action_type,ord_no,cancel_ord_no,event_seq,cancel_event_seq from ARC_OFFER_RELATION_RESTRICTED where 1=1 ";
	
	//	统计总数SQL
	private String SQL_SELECT_COUNT = "select count(*) as col_counts from arc_offer_relation_restricted where 1=1 ";

	//	insert SQl
	private String SQL_INSERT = "insert into ARC_OFFER_RELATION_RESTRICTED (offer_restricted_id, relation_id, product_offer_id, role_type, restricted_type, restricted_product_id, state, state_date, seq, party_id, party_role_id, oper_region_id, oper_date, ord_action_type, ord_no, cancel_ord_no, event_seq, cancel_event_seq) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

	//	普通update SQL
	private String SQL_UPDATE = "update ARC_OFFER_RELATION_RESTRICTED set offer_restricted_id=?, relation_id=?, product_offer_id=?, role_type=?, restricted_type=?, restricted_product_id=?, state=?, state_date=?, seq=?, party_id=?, party_role_id=?, oper_region_id=?, oper_date=?, ord_action_type=?, ord_no=?, cancel_ord_no=?, event_seq=?, cancel_event_seq=? where 1=1 ";
	
	//	普通delete SQL
	private String SQL_DELETE = "delete from ARC_OFFER_RELATION_RESTRICTED where 1=1 ";
	
		//	根据主键delete SQL
	private String SQL_DELETE_KEY = "delete from ARC_OFFER_RELATION_RESTRICTED where offer_restricted_id=? and seq=?";
		
		//	根据主键update SQL
	private String SQL_UPDATE_KEY = "update ARC_OFFER_RELATION_RESTRICTED set offer_restricted_id=?, relation_id=?, product_offer_id=?, role_type=?, restricted_type=?, restricted_product_id=?, state=?, state_date=?, seq=?, party_id=?, party_role_id=?, oper_region_id=?, oper_date=?, ord_action_type=?, ord_no=?, cancel_ord_no=?, event_seq=?, cancel_event_seq=? where offer_restricted_id=? and seq=?";
		
		//	根据主键查询SQL
	private String SQL_SELECT_KEY = "select offer_restricted_id,relation_id,product_offer_id,role_type,restricted_type,restricted_product_id,state,state_date,seq,party_id,party_role_id,oper_region_id,oper_date,ord_action_type,ord_no,cancel_ord_no,event_seq,cancel_event_seq from ARC_OFFER_RELATION_RESTRICTED where offer_restricted_id=? and seq=? ";
	
	//	当前DAO 所属数据库name
	private String dbName = JNDINames.PM_DATASOURCE ;


	public ArcOffRelationRestrictedDAO() {
