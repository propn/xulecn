...
public class ArcProductOffRelationDAO extends AbstractDAO {
	
	//查询SQL
	private String SQL_SELECT = "select relation_id,offer_a_id,offer_z_id,relation_type_id,baseon_type,offer_a_rela_type,offer_z_rela_type,instance_flag,operation_flag,eff_date,exp_date,comp_role_id,role_min_num,role_max_num,relation_desc,time_limit_type,end_time,state,seq,party_id,party_role_id,oper_region_id,oper_date,ord_action_type,ord_no,cancel_ord_no,event_seq,cancel_event_seq,order_id from ARC_PRODUCT_OFFER_RELATION where 1=1 ";
	
	//	统计总数SQL
	private String SQL_SELECT_COUNT = "select count(*) as col_counts from arc_product_offer_relation where 1=1 ";

	//	insert SQl
	private String SQL_INSERT = "insert into ARC_PRODUCT_OFFER_RELATION (relation_id, offer_a_id, offer_z_id, relation_type_id, baseon_type, offer_a_rela_type, offer_z_rela_type, instance_flag, operation_flag, eff_date, exp_date, comp_role_id, role_min_num, role_max_num, relation_desc, time_limit_type, end_time, state, seq, party_id, party_role_id, oper_region_id, oper_date, ord_action_type, ord_no, cancel_ord_no, event_seq, cancel_event_seq, order_id) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

	//	普通update SQL
	private String SQL_UPDATE = "update ARC_PRODUCT_OFFER_RELATION set relation_id=?, offer_a_id=?, offer_z_id=?, relation_type_id=?, baseon_type=?, offer_a_rela_type=?, offer_z_rela_type=?, instance_flag=?, operation_flag=?, eff_date=?, exp_date=?, comp_role_id=?, role_min_num=?, role_max_num=?, relation_desc=?, time_limit_type=?, end_time=?, state=?, seq=?, party_id=?, party_role_id=?, oper_region_id=?, oper_date=?, ord_action_type=?, ord_no=?, cancel_ord_no=?, event_seq=?, cancel_event_seq=?, order_id=? where 1=1 ";
	
	//	普通delete SQL
	private String SQL_DELETE = "delete from ARC_PRODUCT_OFFER_RELATION where 1=1 ";
	
		//	根据主键delete SQL
	private String SQL_DELETE_KEY = "delete from ARC_PRODUCT_OFFER_RELATION where relation_id=? and seq=?";
		
		//	根据主键update SQL
	private String SQL_UPDATE_KEY = "update ARC_PRODUCT_OFFER_RELATION set relation_id=?, offer_a_id=?, offer_z_id=?, relation_type_id=?, baseon_type=?, offer_a_rela_type=?, offer_z_rela_type=?, instance_flag=?, operation_flag=?, eff_date=?, exp_date=?, comp_role_id=?, role_min_num=?, role_max_num=?, relation_desc=?, time_limit_type=?, end_time=?, state=?, seq=?, party_id=?, party_role_id=?, oper_region_id=?, oper_date=?, ord_action_type=?, ord_no=?, cancel_ord_no=?, event_seq=?, cancel_event_seq=?, order_id=? where relation_id=? and seq=?";
		
		//	根据主键查询SQL
	private String SQL_SELECT_KEY = "select relation_id,offer_a_id,offer_z_id,relation_type_id,baseon_type,offer_a_rela_type,offer_z_rela_type,instance_flag,operation_flag,eff_date,exp_date,comp_role_id,role_min_num,role_max_num,relation_desc,time_limit_type,end_time,state,seq,party_id,party_role_id,oper_region_id,oper_date,ord_action_type,ord_no,cancel_ord_no,event_seq,cancel_event_seq,order_id from ARC_PRODUCT_OFFER_RELATION where relation_id=? and seq=? ";
	
	//	当前DAO 所属数据库name
	private String dbName = JNDINames.PM_DATASOURCE ;


	public ArcProductOffRelationDAO() {
