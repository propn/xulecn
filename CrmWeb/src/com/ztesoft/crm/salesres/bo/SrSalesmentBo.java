...
public class SrSalesmentBo{
  public SrSalesmentBo() {
  }

  /**
   * 构造销售资源和撤单资源的错误提示信息
   * 销售和释放资源
   * @param saleList：需要销售的集合
   * @param releseList：需要释放操作的资源
   * @return Map:其中的键信息如下
   * resultFlag:成功或失败的结果，0为失败，1为成功
   * continueFlag：是否继续执行的标志
   * failInfo:错误信息,一般错误时才提取该信息
   * price：营销资源价格信息的map,为营销资源id和价格的map
   */
  public String getSaleReleErrInfo(List saleList,List releseList){
          ...
          if(saleList!=null&&saleList.size()>0){
                  for(int i=0;i<saleList.size();i++){
                      ...
                      if(voTemp!=null){
                              if(voTemp.getRescInstance2()!=null&&voTemp.getRescInstance2().trim().length()>0){
                                      buffer1.append("资源实例"+voTemp.getRescInstance2()+",");
                              }else if(voTemp.getSalesRescId()!=null&&voTemp.getSalesRescId().trim().length()>0){
                                      List listTemp = dao.findByCond(cond+voTemp.getSalesRescId());
                                      if(listTemp!=null&&listTemp.size()>0){
                                              buffer1.append(((SalesRescVO)listTemp.get(0)).getSalesRescName()+",");
                                      }
                              }
                      }
                  }
          ...
          if(releseList!=null&&releseList.size()>0){
                  for(int i=0;i<releseList.size();i++){
                      ...
                      if(voTemp2!=null){
                              if(voTemp2.getRescInstance2()!=null&&voTemp2.getRescInstance2().trim().length()>0){
                                      buffer2.append("资源实例"+voTemp2.getRescInstance2()+",");
                              }else if(voTemp2.getSalesRescId()!=null&&voTemp2.getSalesRescId().trim().length()>0){
                                      List listTemp2 = dao.findByCond(cond+voTemp2.getSalesRescId());
                                      if(listTemp2!=null&&listTemp2.size()>0){
                                              buffer2.append(((SalesRescVO)listTemp2.get(0)).getSalesRescName()+",");
                                      }
                              }
                      }
                  }
  ...
  public Map getSalesRescPriceInfo(List list){
          ...
          for(int i=0;i<list.size();i++){
              ...
              if(voTemp!=null){
                      if(voTemp.getSalesRescId()!=null&&voTemp.getSalesRescId().length()>0){
                              if(voTemp.getBusinessId()==null||voTemp.getBusinessId().length()<1)
                                      map.put(voTemp.getSalesRescId(), "0");
                              else{
                                  priceVO = dao.findByPrimaryKey(voTemp.getBusinessId(), rescLevel, voTemp.getSalesRescId());
                              if(priceVO!=null&&priceVO.getPrice()!=null&&priceVO.getPrice().length()>0)
                                      map.put(voTemp.getSalesRescId(), priceVO.getPrice());
                              else
                                      map.put(voTemp.getSalesRescId(), "0");
                              }
                      }else if(voTemp.getRescInstance2()!=null&&voTemp.getRescInstance2().trim().length()>0){
                         // 根据rescInstance2查询所属营销资源
                              entityVO = entityDao.findByEntityCode(voTemp.getRescInstance2());
                              if(entityVO!=null&&entityVO.getSalesRescId()!=null&&entityVO.getSalesRescId().length()>0){
                                      // 处理流程如上，只是营销资源id不同
                                      if(voTemp.getBusinessId()==null||voTemp.getBusinessId().length()<1)
                                              map.put(entityVO.getSalesRescId(), "0");
                                      else{
                                          priceVO = dao.findByPrimaryKey(voTemp.getBusinessId(), rescLevel, entityVO.getSalesRescId());
                                      if(priceVO!=null&&priceVO.getPrice()!=null&&priceVO.getPrice().length()>0)
                                              map.put(entityVO.getSalesRescId(), priceVO.getPrice());
                                      else
                                              map.put(entityVO.getSalesRescId(), "0");
                                      }
                              }
