...
Control = {
	...
	initDocument : function(){	        
	  ...
	  if(Global.onServer){
			...
			for(var i=0; i<forms_length; i++){	
		        ...
		        if(editors){
		          ...
		          for(var j=0; j<editors_length; j++){
		            ...
		            if(editor && editor.tagName && editor.tagName!="BUTTON" && editor.className && editor.className=="editor"){
		              editor.extra = "editor";
		              initEditor(editor);		  
		            }
		          }
        ...
        //jsDebug.debug("...freshDatasetControls :   :  "+((new Date())-startDateF));    
	...
	uninitElement : function(element){
		...
		switch (_extra){
			...
			case "coolButton":;
			case "customerpilot":{
				if (typeof(Dataset.setElementDataset)!="undefined") Dataset.setElementDataset(element, null);
				if (typeof(element.dataset)!="undefined") element.dataset=null;
				if (typeof(element.record)!="undefined") element.record=null;
				if (typeof(element.field)!="undefined") element.field=null;
				if (typeof(element.dropDown)!="undefined") element.dropDown=null;
				if (typeof(element.table)!="undefined") element.table=null;
				if (typeof(element.editor)!="undefined") element.editor=null;
				if (typeof(element.window)!="undefined") element.window=null;
				if (typeof(element.datapilot)!="undefined") element.datapilot=null;
				if (typeof(element.pilot)!="undefined") element.pilot=null;
				
				if (typeof(element.onmousedown)!="undefined") element.onmousedown=null;
				if (typeof(element.onmouseover)!="undefined") element.onmouseover=null;
				if (typeof(element.onmouseout)!="undefined") element.onmouseout=null;
				if (typeof(element.onkeydown)!="undefined") element.onkeydown=null;
				if (typeof(element.onclick)!="undefined") element.onclick=null;
				if (typeof(element.ondblclick)!="undefined") element.ondblclick=null; 
				if (typeof(element.onkeypress)!="undefined") element.onkeypress=null;
				if (typeof(element.onpropertychange)!="undefined") element.onpropertychange=null;
				break;
			}
		}
	},
	...
	adjustControlsSize : function(){
		if (typeof(Editor.sizeDockEditor)!="undefined"){
			...
			if (typeof(Dropdown.sizeDropDownBtn)!="undefined" && Document.activeEditor) Dropdown.sizeDropDownBtn(Document.activeEditor);
			if (typeof(Dropdown.sizeDropDownBox)!="undefined") Dropdown.sizeDropDownBox();
		}		
	},
					...
					default:{
						...
						if (_dropdown){
							if (_dropdown.type=="list"){
								...
								if (items){
									var item=items.find(["label"], [System.trimStr(element.value)]);
	...
	setElementValue : function(element, value){
				...
				case "string":
					...
					if (_dropdown){
						if (_dropdown.type=="list"){
							...
							if (items){							    
								var item=items.find(["value"], [System.trimStr(value)]);
								if (item){ 
								  ...
								  if(dataset && !dataset.readOnly){
								    if(element.getAttribute("extra")=="editor"){
									  ...
									  if(record){
									    result=record.getValue("label");
									    parentValueId = record.getValue("valueId");
									    if(dataset.getCurrent()){
									      dataset._setValue(element.field, record.getValue("value"));					      
									    }
									  }
									}
								  }
							...
							if(  (element.subList != undefined) && element.subList != "" 
							&& (document.getElementById(element.subList)) && (element.dataset != (document.getElementById(element.subList)).dataset) ){
								//如果没有记录时，设置一个没有存在的parentValueId 
								if(parentValueId == null || parentValueId == "") parentValueId = "__1";
							}
							...
							if((element.subList != undefined) && element.subList != "" && 
							(document.getElementById(element.subList)!=null) && parentValueId != null ){	
								var subEditor = document.getElementById(element.subList);
								...
								//if(!subDropdown.getAttribute("secondTime")){
									...
									&&
									 ( 
									   (element.field=="lanId" || element.field=="businessId" || element.field=="dealExchId" || element.field=="masterExchId" || element.field=="subExchId")
									  ||
									   (element.field=="zLanId" || element.field=="zBusinessId" || element.field=="zDealExchId" || element.field=="zMasterExchId" || element.field=="zSubExchId")
									 )
									){
										//最后一级有关联
										if("DC_SUB_EXCH_ID"==element.dropDown){
						    ...
						    if(result==""){
							  ...
							  if(dataset && !dataset.readOnly){
							    if(dataset.getCurrent()){
								  var defaultValue = dataset.getField(element.field).defaultValue;
							      if(defaultValue && defaultValue!=""){
								  	if(defaultValue=="today"){				    
							        	result = formatDate(new Date(), "yyyy-MM-dd");	
									}
									else if(defaultValue=="firstDateOfThisMonth"){
										result = formatDate(new Date(new Date().getYear(), new Date().getMonth(), 1), "yyyy-MM-dd");
									}
							        dataset._setValue(element.field, result);		
						  ...
						  if(_dropdown.dataset && value==""){
						    ...
						    var dataset = Dataset.getDatasetByID(_dropdown.dataset);
		
						    if(dataset){
						      var field = dataset.getField(element.field).getName();

						      var keyField = element.dataset.getField(element.field).keyField;
						      
						      if(element.tagName=="TD"){
							      var row = Table.getRowByCell(element);
							      if(row && row.record){
								      var keyValue = row.record.getValue(keyField);
								      var record = dataset.locate(keyField, keyValue);
						        ...
						        if(element.dataset){
								      var keyValue = element.dataset.getValue(keyField);
								      var record = dataset.locate(keyField, keyValue);
		...
		switch (element.getAttribute("extra")){	
			case "fieldlabel":{
				var eventName=Document.getElementEventName(element, "onRefresh");
				if (Document.isUserEventDefined(eventName)){
					if (!Document.fireUserEvent(eventName, [element, value])) break;
				}
			...
			case "tablecell":{
				...
				if (Document.isUserEventDefined(eventName)){
					...
					if (record && !Document.fireUserEvent(eventName, [element, value, record])) break;
				}
				...
				if (element.getAttribute("name")=="select") {
					...
					switch (element.getAttribute("editorType")){
						...
						default:{
							...
							var dataset = element.dataset;					
						    if(tmpHTML!="" && dataset && dataset.isFieldMaskControl(element.field)){
						    	element.innerText="######";
						    }		
						    else{
						    	if(tmpHTML!="" && element.editorType=="password"){
						    		var passwordText = "";
		...
		switch (_extra){	
			case "fieldlabel":{
				...
				if (field){
					...
					if (field.required && !field.readOnly && !field.dataset.readOnly){				  					  
					  if(element.id!="" && document.getElementById("_"+element.id)){
			...
			case "columnheader": {
				...
				if (!label){
					if (field){
						...
						label=System.getValidStr(element.getAttribute("name"));
					}
				}
				if (!label){
				...
				if (field){
					if (System.isTrue(field.required) && !field.readOnly && !field.dataset.readOnly){
			...
			default:{
				...
				if (dataset){
					...
					if (fieldName) {
						if ((_extra=="editor" || _extra=="dockeditor") && Document.forEditor==element) {
...
Document = {
  ...
  //    判断该事件是否有响应函数，如果有设定defined属性为真（并且handle属性为函数名），否则为假
	...
	fireUserEvent : function(function_name, param){
		...
		var eventInfo=Document.user_events[function_name];
		if (eventInfo==null) {
			if (!Document.isUserEventDefined(function_name)) return;
	...
	processActiveElementChanged : function(activeElement){
		function isChildofTable(obj) {
			...
			if (tmpObj.getAttribute("extra")=="tablecell") result=Table.getTableByCell(tmpObj);
			return result;
		}
				...
				if (Document.activeEditor){
					if (Editor.needUpdateEditor){
						if (Document.activeEditor.window==window)
							...
							Document.activeEditor.window.Editor.updateEditorInput(Document.activeEditor);
					} 
					if (typeof(Dropdown.hideDropDownBtn)!="undefined") Dropdown.hideDropDownBtn();
				...
				if (editor && !editor.readOnly){
					...
					if (editor.getAttribute("extra")=="editor" || editor.getAttribute("extra")=="dockeditor"){
						...
						if (!editor.getAttribute("dropDown") &&
							...
							|| editor.getAttribute("dataType")=="time")){
							//防止没有初始化日期
							makeCalendar(); 
						...
						if (editor.getAttribute("extra")=="editor" && field){
							...
							if (field.size > 100 && System.compareText(editor.tagName, "TEXTAREA") &&
								!editor.getAttribute("dropDown")){
								Document.stored_element=editor;
								editor.editorType="textarea";
								setTimeout("Editor.showDockEditor(Document.stored_element);", 0);
							}
						}
					}
					...
					if (_dropdown){
						editor.contentEditable=(!System.isTrue(_dropdown.fixed));
						if (typeof(Dropdown.showDropDownBtn)!="undefined"){
							if (_dropdown && System.isTrue(_dropdown.autoDropDown)) Dropdown.showDropDownBox(editor);
						}
					}
					else{
					...
					if (!(_dropdown && System.isTrue(_dropdown.fixed)) &&
						!System.compareText(editor.type, "checkbox")) editor.select();
				}
		...
		function processElementBlur(){		  
			...
			if (Document.activeElement){
				if (typeof(_dropdown_btn)!="undefined" && _dropdown_btn){
					doblur=doblur && (Document.activeElement!=_dropdown_btn) &&
				...
				if (typeof(Dropdown.box)!="undefined" && Dropdown.box){
					...
					doblur=doblur && (activeElement!=editor) &&
						(!Document.isChild(activeElement, Dropdown.box));
				}
	
				if (doblur){
					if (Document.activeEditor && Document.activeEditor.dropDownVisible){
						if (typeof(Dropdown.hideDropDownBox)!="undefined") Dropdown.hideDropDownBox();
			...
			if (activeElement==document.body && Document.skip_activeChanged){
				Document.skip_activeChanged=false;
				return;
			}
			}
			if ((doblur || !Document.activeEditor) && (activeElement.getAttribute("extra")=="tablecell")){
				...
				if (Document.activeTable!=activeTable){
					if (Document.activeTable){
						...
						if (row) Datatable.refreshTableRowStyle(row);
					}
			...
			if (Document.activeElement){
				if (typeof(Menu)!="undefined" && typeof(Menu.hideMenu)!="undefined"){
					...
					case "tablecell":{
						...
						if (row.record){
							if (dataset.window==window){								
								...
								dataset.window.Dataset.setRecord(dataset, row.record);
							}
						}	
	...
	getPriorTabElement : function(obj){ 
		...
		while (i<elementCount){
			...
			if (tmpObj!=obj) 
			{
				...
				case "TEXTAREA":
				case "BUTTON":
					if (tmpObj.tabIndex!=-1 && !tmpObj.disabled && !System.isTrue(tmpObj.readOnly))
					{
						return tmpObj;
					}
				case "TD":
					if (tmpObj.extra=="tablecell" && !System.isTrue(tmpObj.readOnly))
					{
						return tmpObj;
					}
				}
	...
	control_onkeydown : function() {
		...
		if (Dropdown.isDropdownBoxVisible()){
			if (Dropdown.window) Dropdown.window.Dropdown.processDropDownKeyDown(event.keyCode);
			...
			switch (event.keyCode) {
				...
				case 13:{
				  if(element.dropDown){
				    Dropdown.hideDropDownBox(element);
				  ...
				  else if (Global.processEnterAsTab && !System.compareText(element.tagName, "TEXTAREA") || event.shiftKey || event.ctrlKey || event.altKey){
						...
						if (cell && !event.shiftKey){
							...
							if (row.rowIndex==maxIndex[0] && cell.cellIndex==maxIndex[1] && !System.isTrue(table.getAttribute("readOnly"))){
								var dataset=Control.getElementDataset(element);
								dataset.insertRecord("end");
								//dataset.modified=false;
								Datatable.setActiveTableCell(table.activeRow, 0);
							}
							else{
								processTab(element);
							}
				...
				case 37:{
					...
					if (cell){
						if ((event.ctrlKey) || (event.altKey)){
							...
							cellIndex--;
							Editor.setFocusTableCell(table, rowIndex, cellIndex);
							event.returnValue=false;
						}
					}
					break;
				  ...
				  if(!element.readOnly && element.dropDown){
          		  ...
          		  {          		  	
						...
						if (cell){
							var dataset=Control.getElementDataset(element);
							if (dataset){
							    if (dataset.getCurrent() && dataset.getCurrent().getPrevRecord())
									dataset.movePrev();
								event.returnValue=false;
							}
						}
				...
				case 40:{
				  ...
				  if(!element.readOnly && element.dropDown){					
						...
						if (cell){
							...
							var dataset=Control.getElementDataset(element);
							if (dataset){
							    if (dataset.getCurrent() && dataset.getCurrent().getNextRecord())
									dataset.moveNext();
								/*
								if (dataset.isLast() && !System.isTrue(table.getAttribute("readOnly")) && !System.isTrue(dataset.readOnly)){
									dataset.insertRecord("end");
									//dataset.modified=false;
								}
								*/
				...
				case 45:{
					var cell=getCell(element);
					if (cell && !System.isTrue(Table.getTableByCell(cell).getAttribute("readOnly"))){
						var dataset=Control.getElementDataset(element);
						if (!System.isTrue(dataset.readOnly)){
				...
				case 46:{
					if (event.ctrlKey){
						...
						if (cell){
							var table=Table.getTableByCell(cell);
							if (!System.isTrue(table.getAttribute("readOnly"))){
								var dataset=Control.getElementDataset(element);
								if (!System.isTrue(dataset.readOnly)){
									if (System.isTrue(table.getAttribute("confirmDelete"))){
										if (confirm(Const.DatasetConfirmDelete)){
...
document.onkeydown = function(){ 
	if (Global.onServer && System.isTrue(Global.disableContextRefresh) && event.keyCode==116) 
	{ 
		event.keyCode = 0; 
		event.cancelBubble = true; 
		return false; 
	}
    else if(document.activeElement && document.activeElement.tagName!="BUTTON" && event.keyCode==13){
...
window.onunload = function() {
	...
	for(var i=0; i<Document.array_dataset.length; i++){
		...
		if (dataset.window==window){
		  ...
		  dataset.window=null;
		  if (typeof(dataset.datapilot)!="undefined") dataset.datapilot=null;
		  if (typeof(dataset.pilot)!="undefined") dataset.pilot=null;
	...
	for(var i=0; i<_array_dropdown.length; i++){
	    ...
	    if(dropdown){
	      if (typeof(dropdown.table)!="undefined") dropdown.table=null;
		  if (typeof(dropdown.window)!="undefined") dropdown.window=null;
		  if (typeof(dropdown.dropdownbox)!="undefined") dropdown.dropdownbox=null;
		  dropdown = null;
	    }
	}
...
window.confirm = function(message){
  return (MessageBox.Show(null,message,null,'OKCancel','Asterisk')=="OK" ? true : false);
}

window.confirm2 = function(message){
  return (MessageBox.Show(null,message,null,'YesNo','Asterisk')=="Yes" ? true : false);
}
window.confirm3 = function(message){
  return (MessageBox.Show(null,message,null,'YesNoCancel','Asterisk')=="Yes" ? true : false);
}
...
function doError() {
	if (!w || w.closed) {
		...
		var top = (y - 124)/2;
		var left = (x - 400)/2;
		w = window.open(path_prefix+"public/components/messageBox/error.htm","_webxf_error_win","width=385,height=80,top="+top+",left="+left+"");
		w.focus();
	}
	return true;
}
...
function openDialog(url, arg, sizeConfig, dialogWidth, dialogHeight){
  ...
  if(!sizeConfig || sizeConfig == ""){   
    sizeConfigResult = "dialogWidth:"+dialogWidth+"px;dialogHeight:"+dialogHeight+"px;";
  }else{
    var __config = eval("__"+sizeConfig);
    sizeConfigResult = "dialogWidth:"+__config[0]+"px;dialogHeight:"+__config[1]+"px;";
  }
  return window.showModalDialog(url, arg, "status:no;help:no;scroll:no;"+sizeConfigResult);
}
//显示悬浮弹出窗口
function openModelessDialog(url, arg, sizeConfig, dialogWidth, dialogHeight){
  ...
  return window.showModelessDialog(url, arg, "status:no;help:no;scroll:no;"+sizeConfigResult);
}
...
var Editor = {
	...
	isReadOnly : function() { return System.isTrue(this._readOnly); },	
	getDropDown : function() { return this.dropDown; },
	
	checkbox_onclick : function() { Editor.processEditorValueChanged(event.srcElement); },	
	
	onpropertychange : function() {
		if (event.propertyName=="value"){
			var editor=event.srcElement;
			if(Document.activeEditor){
				if ( Document.activeEditor==editor) Editor.processEditorValueChanged(editor);
			}
		}
	},		
	onkeypress : function() {
		...
		if(editor){
			with (editor){
				if (_readOnly){
					...
					if(editor.dropDown){
		    			style.background = "url("+path_prefix+"public/skins/bsn/downdot.gif) top right no-repeat white";
						style.cursor = "hand";
					}
	...
	processDropDownSelected : function(editor, record){
		...
		needAbort=(Document.isUserEventDefined(eventName) && !Document.fireUserEvent(eventName, [_dropdown, record, editor]));
		if (needAbort) return;
			...
			default:{
				...
				if (dataset){
					if (fieldmaps.length>0)	{
						for(var i=0; i<fieldmaps.length; i++){
							if (record) {
								dataset.setValue(fieldmaps[i].writeField,
										record.getValue(fieldmaps[i].readField));
							}
							else {
								dataset.setValue(fieldmaps[i].writeField, "");
							}
						...
						if (record) {
							if (_dropdown.type=="list") {
								Control.setElementValue(editor, record.getValue("value"));
							}
							else if (dataField){
	...
	validEditorInput : function(editor){
		...
		var _dropdown=Editor.getEditorDropDown(editor);
		if (!editor.value || (_dropdown && _dropdown.type=="list")) return;
	...
	updateEditorInput : function(editor){
		try{
			...
			if (editor.modified){
				...
				var eventName=Document.getElementEventName(editor, "onUpdate");
				needAbort=(Document.isUserEventDefined(eventName) && !Document.fireUserEvent(eventName, [editor]));
				if (needAbort) return;
					...
					if (editor.value!=""){
						...
						switch (_dropdown.type){
							...
							case "dataset":{
								...
								if (tmp_dataset){
									if (typeof(tmp_dataset)=="string") tmp_dataset=Dataset.getDatasetByID(tmp_dataset);
									if (tmp_dataset){
										var dataField=_dropdown.dataField;
										if (!dataField) dataField=editor.getAttribute("field");
										if (dataField){
											var record=tmp_dataset.find([dataField], [editor.value]);
											notInList=(record==null);
											if (!notInList)	Editor.processDropDownSelected(editor, record);
										}
									}
								}
								break;
				...
				if (dataset && dataset.record){
					//if (dataset.window==window){
						dataset.record.setValue(dataField, System.trimStr(editorValue));
					//}
					//else{
					//	dataset.window.Record._setValue(dataset.record, dataField, System.trimStr(editorValue));
					//}
				}
			}
	...
	refreshSubEditor: function(editor) {
		...
		if(subEditor==null || subEditor==undefined) return;
		...
		var parentValueId = Editor.getEditorDropDown(editor)._items.getValue("valueId");
		if(subDropdown.getAttribute("configParams")!=null){
			var subConfigParams = subDropdown.getAttribute("configParams")+parentValueId;
		...
		if(parentValueId!=null){		
			...
			if(subDropdown.defaultValue && subDropdown.defaultValue!=null){
				...
				subDataset.setRecord(subDataset.locate("value", subDropdown.defaultValue));
			...
			if(dataset){
			  ...
			  dataset._setValue(subEditor.field, subDropdown._items.getValue("value"));
			}
		}
	...
	setSubEditor: function(editor, parentValueId) {
		...
		if(subConfigParams!=null){
			subConfigParams = subDropdown.getAttribute("configParams")+parentValueId;
			subDropdown.setAttribute("configParams", subConfigParams);
		}else{
	...
	getEditorDropDown : function(editor) {
		...
		/*
		if(editor.tagName=="TD" && editor.dataset.getCurrent() && editor.dataset.id=="assureInfo" && editor.dataset.getValue("field11")=="0001"){
		  alert(11);
      _dropdown = "assureContentInfo";
		}else{
		  alert(editor.tagName);
		  alert(editor.dataset.getCurrent());
		  alert(editor.dataset.id);
		  alert(editor.dataset.getValue("field11"));
		  alert(10);
		}
		*/
	...
	sizeDockEditor : function(editor) {
		var _editor=(editor)?editor:((Document.activeEditor)?(Document.activeEditor):false);
		if (!_editor) return;
	...
	showDockEditor : function(holder, noCheck){
		if(!noCheck){
			if (System.isTrue(holder.getAttribute("readOnly"))) return;//throw "abort";
			var _dataset=Control.getElementDataset(holder);
			if (!Editor.isDataEditorEditable(holder, _dataset)) return;//throw "abort";
		}
			...
			with (editor){
				if (style.visiblity!="visible"){
					...
					if(holder.getAttribute("maxLength")!=null && holder.getAttribute("maxLength")!="")
		...
		switch (editorType){
			case "textarea":{
				if  (typeof(_table_textarea)=="undefined"){
					result=document.createElement("<TEXTAREA id=_table_textarea class=dockeditor extra=dockeditor tabindex=-1"+
			...
			case "checkbox":{
				if  (typeof(_table_checkbox)=="undefined"){
					result=document.createElement("<INPUT id=_table_checkbox type=checkbox hidefocus=false class=dockeditor extra=dockeditor tabindex=-1"+
			...
			case "password":{
				if  (typeof(_table_passwordeditor)=="undefined"){
					result=document.createElement("<INPUT id=_table_passwordeditor type=password class=dockeditor extra=dockeditor tabindex=-1"+
						" style=\"position: absolute; display: none\">");
					document.body.appendChild(result);
	        		initEditor(result);
	    			result.onfocus=function(){        			
	        			var result= openDialog("/VsopWeb/cs/boa/password.jsp", null, "", "300px", "200px");
				...
				if  (typeof(_table_dockinputeditor)=="undefined"){
					result=document.createElement("<INPUT id=_table_dockinputeditor class=dockeditor extra=dockeditor tabindex=-1"+
						" style=\"position: absolute; display: none\">");
					document.body.appendChild(result);
	        		initEditor(result);
	    			result.onfocus=function(){  
	        			...
	        			var result= openDialog("/VsopWeb/cs/boa/inputText.jsp", [key,dataset], "", "600px", "400px");
	        			/*if(typeof(result)!="undefined"&&result!=null&&result!=""){
	        				this.value=result;
	        			}*/
	        			this.blur();
				...
				if  (typeof(_tplPop_table_dockinputeditor)=="undefined"){
					result=document.createElement("<INPUT id=_tplPop_table_dockinputeditor class=dockeditor extra=dockeditor tabindex=-1"+
						" style=\"position: absolute; display: none\">");
					document.body.appendChild(result);
	        		initEditor(result);
	    			result.onfocus=function(){  
	        			...
	        			var result= openDialog("/VsopWeb/cs/tpl/component/tplAttrInputCommon.jsp", [key,dataset], "", "600px", "400px");
	        			/*if(typeof(result)!="undefined"&&result!=null&&result!=""){
	        				this.value=result;
	        			}*/
	        			this.blur();
				...
				if  (typeof(_tplPopDown_table_dockinputeditor)=="undefined"){
					result=document.createElement("<INPUT id=_tplPopDown_table_dockinputeditor class=dockeditor extra=dockeditor tabindex=-1"+
						" style=\"position: absolute; display: none\">");
					document.body.appendChild(result);
	        		initEditor(result);
	    			result.onfocus=function(){  
	        			...
	        			var result= openDialog("/VsopWeb/cs/tpl/component/tplAttrInputCommon.jsp", [key,dataset,window], "", "600px", "400px");
	        			/*if(typeof(result)!="undefined"&&result!=null&&result!=""){
	        				this.value=result;
	        			}*/
	        			this.blur();
			...
			case "ipeditor":{
				if  (typeof(_table_ipeditor)=="undefined"){
					result=document.createElement("<INPUT id=_table_ipeditor class=dockeditor extra=dockeditor tabindex=-1  size='15' maxlength='15' onFocus='' onpaste='checkPaste()' onDrag='checkPaste()' oncut='checkPaste()' onmousemove='checkPaste()'"+
						" style=\"position: absolute; display: none\">");
					document.body.appendChild(result);
	        		initEditor(result);
	        		result.onkeydown=function(){keydown(this);}
	        		result.onkeypress=function(){keypress(this);}
	        		result.onbeforedeactivate=function(){return checkip(this);}
				}
				else{
			...
			case "date":{
				if  (typeof(_table_dateeditor)=="undefined"){
					result=document.createElement("<INPUT id=_table_dateeditor class=dockeditor extra=dockeditor tabindex=-1"+
						" style=\"position: absolute; display: none\">");
					document.body.appendChild(result);
	        		initEditor(result);
	        		//result.onkeydown=function(){validateDatetime(this,1)}
	        		result.title="格式为：9999-12-31"
	        		result.onbeforedeactivate=function(){return validateDatetimeFormat(this, 1);}
				}
				else{
			...
			case "time":{
				if  (typeof(_table_timeeditor)=="undefined"){
					result=document.createElement("<INPUT id=_table_timeeditor class=dockeditor extra=dockeditor tabindex=-1"+
						" style=\"position: absolute; display: none\">");
					document.body.appendChild(result);
	        		initEditor(result);
	        		//result.onkeydown=function(){validateDatetime(this,2)}
	        		result.title="格式为：23:59:59"
	        		result.onbeforedeactivate=function(){return validateDatetimeFormat(this, 2);}
				}
				else{
			...
			case "popDate":{
				if  (typeof(_table_dockdateinputeditor)=="undefined"){
					result=document.createElement("<INPUT id=_table_dockdateinputeditor class=dockeditor extra=dockeditor tabindex=-1"+
						" style=\"position: absolute; display: none\">");
					document.body.appendChild(result);
	        		initEditor(result);
	    			result.onfocus=function(){  
	        			var result= openDialog("/VsopWeb/cs/boa/date.jsp", "", "", "320px", "250px");
	        			if(typeof(result)!="undefined"&&result!=null&&result!=""){
	        				this.value=result;
	        			}
	        			this.blur();
			...
			case "datetime":{
				if  (typeof(_table_datetimeeditor)=="undefined"){
					result=document.createElement("<INPUT id=_table_datetimeeditor class=dockeditor extra=dockeditor tabindex=-1"+
						" style=\"position: absolute; display: none\">");
					document.body.appendChild(result);
	        		initEditor(result);
	        		//result.onkeydown=function(){validateDatetime(this,3)}
	        		result.title="格式为：9999-12-31 23:59:59"
	        		result.onbeforedeactivate=function(){return validateDatetimeFormat(this, 3);}
				}
				else{
			...
			default:{
				if  (typeof(_table_texteditor)=="undefined"){
					result=document.createElement("<INPUT id=_table_texteditor class=dockeditor extra=dockeditor tabindex=-1"+
				...
				if(editorType=="double"){
					result.onbeforedeactivate=function(){
						if(this.value!="" && !/^[-\+]?\d+(\.\d+)?$/.test(this.value)){
							...
							if(!Editor.checkValue(minValue,maxValue,valueMinLen,valueMaxLen,this.value)){
								this.value = "";
								this.focus();
								event.returnValue = false;
							}
					...
					result.onbeforedeactivate = function(){
						if(!Editor.checkValueByRule(checkRule,checkRuleType,checkRuleHint,this.value)){
							this.value = "";
							this.focus();
							event.returnValue = false;
						}
    ...
    initDockEditor : function(){
		...
		result=document.createElement("<INPUT id=_popup_texteditor class=dockeditor extra=dockeditor tabindex=-1"+
			" style=\"position: absolute; display: none\">");
	...
	checkElementInHidden : function(element){
		...
		if(typeof(element.parentElement)!="undefined" && element.parentElement.nodeName!="BODY"){
...
function initEditor(editor){   
    ...
    if( ((editor.extra == "editor") && editor.dropDown && !editor.readOnly)
      || (field && (field.dataType=="date") && (editor.extra=="editor") && !editor.readOnly)){
      editor.style.background = "url("+path_prefix+"public/skins/bsn/downdot.gif) top right no-repeat white";
	  editor.style.cursor = "hand";  
    }
  
    editor.onmousedown=function(){if(!editor.readOnly) return Dropdown.btn_onmousedown2(this);};  
