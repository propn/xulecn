	...
	function save_onClick(){
		...
		var TSecondVO = extractBeanFromDataset(dsSecondForm, 'fortest.nxa.vo.TSecondVO');
		var TMasterVO = extractBeanFromDataset(dsMasterForm, 'fortest.nxa.vo.TMasterVO');
		
		NDAjaxCall.getSyncInstance().remoteCall('NXAService',actionType,
			[TMasterVO,TSecondVO],function(replay){
			jsDebug.debug("nxa page "+actionType+" data cost time="+(new Date()-s));
	...
	function delete_onClick(){
		...
		NDAjaxCall.getAsyncInstance().remoteCall("NXAService", "delete",[id,sid],function(r){
			jsDebug.debug("nxa page delete data cost time="+(new Date()-s));
			if(r.getResult()){
				queryData();
			}else{
				alert('操作失败，发生异常');
