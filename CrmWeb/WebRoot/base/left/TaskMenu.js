...
var CFG_DOCUMENT_MARGIN = 1;
var CFG_MENU_SPACING = 10;
var CFG_MENU_WIDTH = 148;
	...
	this.setCommand = function(strCommand) {
		if ((this.command = strCommand) != null) {
			...
			JS.removeListener(this.hLabel, "mouseout",
					TaskMenuItem.onMouseBehavior, false);
			JS
					.removeListener(this.hLabel, "click", TaskMenuItem.onClick,
							false);
		...
		with (this.menuBody = document.createElement("DIV")) {
			...
			style.cssText = "border-width:0px "
					...
					+ ((this.background != null) ? "background-image:url('"
							+ this.background + "');" : "");
			style.top = (parseInt(this.titlebar.style.top) + CFG_TITLEBAR_HEIGHT)
					+ "px";
			style.left = CFG_DOCUMENT_MARGIN + "px";
			style.width = (CFG_MENU_WIDTH
					+ ((INF_IS_IEXPLORER) ? 0
