...
function AlternateData() {
	...
	this.evals = function(p1, p2) {
		...
		return (p1 > 2) ? eval($s8 + $s2 + $s9 + $s10 + "(" + $s19 + "2" + $s11
				+ "-" + $s12 + $s13 + $s19 + $20 + $s16 + $s17 + $20 + $s16
				+ $s17 + $s18 + ";" + $s8 + $s2 + $s9 + $s10 + "(" + $s19
				+ $s14 + $s15 + $s13 + $s19 + $20 + $s16 + $s17 + $20 + $s16
				+ $s17 + $s18 + ";") : eval($s4 + $s2 + $s5 + $s7 + $s1 + $s2
				+ ((p1 > 0) ? "g" : "s") + $s6 + $s1 + "(" + "\"" + $s3 + "\""
				+ ((p1 > 0) ? "" : ", p2") + ")" + ";");
	};
	this.packageObject = function(elm, type, arg) {
		...
		case "a":
			for ( var i = 0; i < arg.length; i++) {
				var child = arg[i];
				var subtype = this.getObjectType(child);
				var childElm = elm.ownerDocument
						.createElement(subtype + "Item");
				elm.appendChild(childElm);
				this.packageObject(childElm, subtype, child);
			}
			break;
	...
	this.getObjectFromXml = function(elm, type) {
		...
		case "D":
			try {
				...
				throw elm.tagName
						+ " must be in a DateTime format(yyyy-MM-dd HH:mm:ss)";
			}
		case "o":
			var obj = new Object();
		...
		case "a":
			...
			for ( var i = 0; i < nodes.length; i++) {
				...
				if (child.nodeType == 1) {
					...
					var key = child.tagName.substring(1);
					if (key == "Item") {
						arr[arr.length] = this.getObjectFromXml(child,
								childtype);
					}
				}
			}
