	...
	protected void doRequest(HttpServletRequest request,
		...
		try {// 增加异常处理
			...
			if (cause instanceof InvocationTargetException) {
				...
				while (currentException instanceof InvocationTargetException) {
					InvocationTargetException ie = ((InvocationTargetException) currentException);
					currentException = (Exception) ie.getTargetException();
				}
				if (currentException instanceof CrmException) {
					crmEx = (CrmException) currentException;
				}else {
					CommonError error = new CommonError(CommonError.COMMON_ERROR);
					crmEx = new CommonException(error, currentException);
				}
			} 
			//modify by AyaKoizumi 100922
			if ( !(cause instanceof CrmException) && crmEx==null) {//session失效提示控制 2010-09-12
				CommonError error = new CommonError(CommonError.COMMON_ERROR);
				crmEx = new CommonException(error, ex);
			}
			ExceptionVO vo = new ExceptionVO(crmEx);
			StringBuffer exceptionBuffer = new StringBuffer();
			exceptionBuffer.append("<buffalo-reply><m><t>").append(
					vo.getClass().getName()).append("</t><s>errorCode</s><s>");
			exceptionBuffer.append(vo.getErrorCode()).append(
					"</s><s>errorMessage</s><s><![CDATA[").append(
					vo.getErrorMessage());
			exceptionBuffer.append(
					"]]></s><s>errorResolve</s><null></null><s>level</s><i>")
					.append(vo.getLevel());
			exceptionBuffer.append("</i><s>stackInfo</s><s><![CDATA[").append(
					vo.getStackInfo()).append("]]></s></m></buffalo-reply>");
			response.setContentType("text/html;charset=GBK");
			OutputStreamWriter w = new OutputStreamWriter(response
					.getOutputStream());
