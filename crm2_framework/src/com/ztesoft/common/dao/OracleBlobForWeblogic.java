...
public class OracleBlobForWeblogic {
	...
	/**
	 * 存储Blob字段，仅仅对Oracle数据库有用 如果处于两层调试阶段，那么需要对Connection设置为非自动提交
	 * 
	 * @param rs
	 *            预处理记录集
	 * @param index
	 ...
	 * @throws SQLException
	 */
	public static void setBlob(ResultSet rs, int index, byte[] buffer) throws SQLException {
		...
		try {
			...
			if (blob instanceof weblogic.jdbc.vendor.oracle.OracleThinBlob) {
				OracleThinBlob wrapBlob = (OracleThinBlob) blob;
				outstream = wrapBlob.getBinaryOutputStream();
				...
				// }
					...
					throw new SQLException("Unable to handle concrete type of Blob, " + blob.getClass().getName());
				}
			}
			if (outstream != null) {
	...
	public static void setOracleBlob(ResultSet rs, int index, byte[] buffer) throws SQLException {
		OutputStream outstream = null;
		...
		try {
			...
			if (blob instanceof oracle.sql.BLOB) {
				outstream = ((oracle.sql.BLOB) blob).getBinaryOutputStream();
			} else if (blob instanceof weblogic.jdbc.vendor.oracle.OracleThinBlob) {
				OracleThinBlob wrapBlob = (OracleThinBlob) blob;
				outstream = wrapBlob.getBinaryOutputStream();
	 ...
	 * @throws SQLException
	 */
	public static byte[] getOracleBlob(ResultSet rs, int index) throws SQLException {
		Blob blob = rs.getBlob(index);
	 ...
	 * @throws SQLException
	 */
	public static byte[] getBlob(ResultSet rs, String fieldName) throws SQLException {
