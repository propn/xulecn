...
public class TfmServicesDAO extends AbstractDAO {
	...
	//	insert SQl
	private String SQL_INSERT = "insert into TFM_SERVICES (service_name, env_id, version, service_desc, service_type, state, cache_flag, module_name, if_log) values (?, ?, ?, ?, ?, ?, ?, ?, ?)";

	//	普通update SQL
	private String SQL_UPDATE = "update TFM_SERVICES set service_name=?, env_id=?, version=?, service_desc=?, service_type=?, state=?, cache_flag=?, module_name=?, if_log=? where 1=1 ";
	...
	private String SQL_DELETE_KEY = "delete from TFM_SERVICES where service_name=?";
		...
		//	根据主键update SQL
	private String SQL_UPDATE_KEY = "update TFM_SERVICES set service_name=?, env_id=?, version=?, service_desc=?, service_type=?, state=?, cache_flag=?, module_name=?, if_log=? where service_name=?";
		
		//	根据主键查询SQL
	private String SQL_SELECT_KEY = "select service_name,env_id,version,service_desc,service_type,(select class_name from tfm_serv_class_def d where d.service_name=s.service_name ) class_name,state,cache_flag,module_name,if_log from TFM_SERVICES s where s.service_name=? ";
