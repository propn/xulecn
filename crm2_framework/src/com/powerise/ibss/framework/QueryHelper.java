...
public class QueryHelper {
	...
	// 执行SELECT语句，返回DynamicDict对象：追加或直接返回方式；正常或HashMap方式 add by AyaKoizumi 101008
	public static DynamicDict queryPageNoCount(String jndiName, String querySQL, List para, int pageIndex,
			int pageSize, DynamicDict dto, String name, int ecode, int etype, String edesc) throws FrameException {
		jndiName = jndiName.toUpperCase();
		if (dto == null) // 装入对象
			dto = new DynamicDict();
...
//	 * SQL SELECT -> ResultSet
//	 */
//	private static ResultSet sql__query2(PreparedStatement st,String sql, List para)
//			throws SQLException, FrameException {
//		if (para != null) {
//			String sqlTemp = new String(sql);
//
...
//					+ sqlTemp.substring(sqlTemp.indexOf("?") + 1, sqlTemp.length());
//			}
//			System.out.println("查询结果信息列表:=====================" + sqlTemp);
//		}
//
//		if (para != null){
...
//	 * SQL SELECT -> ResultSet
//	 */
//	private static ResultSet sql__query4(PreparedStatement st, String sql, String[] para) throws SQLException, FrameException {
//		if (para != null) {
//			String sqlTemp = new String(sql);
//
//			for (int i = 0; i < para.length; i++) {
		...
		while (rs.next()) {
			...
			for (int j = 1; j <= rsm.getColumnCount(); j++) {
				...
				if ("DATE".equals(columnType)) {
					...
					val = DAOUtils.getFormatedDateTime(rs.getTimestamp(j));
				} else if("CLOB".equals(columnType) && "VsopWeb".equals(appName)){//vsop 新增
					val = DAOUtils.ClobToString(rs.getClob(j));
				}else {
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (int i = 0; i < 100; i++) {
				...
				DynamicDict dto = QueryHelper
						.queryPage(
								...
								"select count(1) from dc_public p where  stype=? and pkey like '%'||?||'%'", para,
			...
			for (int i = 0; i < 100; i++) {
				...
				// String whereCond = " 1=1 and stype='"+para[0]+"' and pkey
				// like '%'||'"+para[1]+"'||'%' " ;
				// PageModel pageModel2 = PageHelper.popupPageModel(dao,
				// whereCond, pageIndex, pageSize);
			}
