...
Global = {
	...
	//context path
	contextPath : SystemContext.appContextPath,
	
	//应用服务器路径
	serverPath : SystemContext.appBuffaloServlet,  
  ...
  switch(library){
    case "kernel" :{
      if(!Global.onServer){	
			...
			CrmComponent._import("js",       "public/components/environment.js");	
...
window.onload = function(){
	    ...
	    setTimeout("Control.initDocument();", 0);
  		...
  		//系统功能菜单的特殊处理 ,2.0特有功能,加条件限制
  		if('CrmWeb' == SystemContext.appName){
  			...
  			try{
	  			if(top.mainFrame&&top.mainFrame.document){
					...
					for(var i=0;i<divs.length;i++){
						...
						if(className=='b-m-mpanel'||className=='switchLanDiv'|| className.indexOf('hideNullClass')>-1 ){
...
//=============================================PPMWeb新增=======================================================
function btnOnClick(btn , elementEventName , datasetId , fieldName,actionId){
	if(btn.disabled) 
		return false;
	if(elementEventName){
		...
		//if (Document.isUserEventDefined(elementEventName)){
			var fieldValue='';
			if(fieldName.indexOf('name_field_')>-1){
				fieldValue=fieldName.substr(5,fieldName.length);
			}
			Document.fireUserEvent(elementEventName, [datasetId, fieldName,fieldValue,actionId]);
		//}
...
function showTip() { 
 ...
 if(typeof(altlayer)=="undefined" || altlayer==null){
	var div = document.createElement("div");
	div.id = "altlayer";
	//div.style="visibility:hidden;border:2px;padding:5px;font-size:12px;background-color:#990000;position:absolute;";
...
///--结束
function copyData(srcDataSet, tarDataSet) {
	tarDataSet.disableControls();
	tarDataSet.clearData();
	tarDataSet.insertRecord(null);
	tarDataSet.setReadOnly(true);
	for (var i = 0; i < srcDataSet.fields.length; i++) {
		var fieldName = srcDataSet.fields[i].name;
		if (tarDataSet.getField(fieldName)) {
			tarDataSet.setValue(fieldName, srcDataSet.getValue(i));
		}
	}
	tarDataSet.enableControls();
}
