...
//        var newUrl = this.gateway + "/buffalo/" + serviceMethodPair[0] ;
        ...
        //extend code
        //this.localURL = serviceMethodPair[0] + "." + serviceMethodPair[1] + "." + call.xmlString() + ".xml";
        if (!Global.onServer) {
...
//        if (this.transport.responseText && this.transport.status == '200' ) {
        //extend
        if (!Global.onServer ||(this.transport.responseText && this.transport.status == '200') ) {
...
NDAjaxCall.__remoteCall = function(ndajax, serviceName, methodName, params, callBack, replyType, fields) {
    ...
    if (System.isTrue(ndajax.async) && browser == "Microsoft Internet Explorer" && document.readyState != "complete") {
        window.setTimeout(function() {
            NDAjaxCall.__remoteCall(ndajax, serviceName, methodName, params, callBack, replyType, fields);
        }, 5);
        return false;
    }
