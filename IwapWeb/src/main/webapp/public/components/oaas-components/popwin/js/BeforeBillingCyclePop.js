...
{
	var tempCycDto = window.dialogArguments;	
	...
	var list = findBeforeBCList(dQryDateBegin,dQryDateEnd);
	if(list == null) {
		document.all("billingCycleTree").clear();
		return;
	}
  ...
  if(tempCycDto != null){
  	for(var i=0;i<allItems.length;i++){
  		if(tempCycDto.Id == allItems[i].billingCycleId){
