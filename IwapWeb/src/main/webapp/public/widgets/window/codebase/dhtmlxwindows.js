...
function dhtmlXWindows() {
	...
	this._autoResizeViewport = function() {
		...
		for (var a in this.wins) {
			...
			var skinParams = (win._skinParams != null
					? win._skinParams
	...
	this._parkWindow = function(win, parkBeforeMinMax) {
		...
		if (win._isParked) {
			...
			if (this.enableParkEffect && !parkBeforeMinMax) {
				win.lastParkH = (String(win.h).search(/\%$/) == -1
						? win.h
						: win.offsetHeight);
				if (win._allowResizeGlobal == true) {
	...
	this._closeWindow = function(win) {
		if (this._focusFixIE) {
			this._focusFixIE.style.top = (this.vp == document.body
					? 0
...
dhtmlXWindows.prototype._dhx_Engine = function() {
	...
	this._engineSetWindowBody = function(win) {
		win.innerHTML = "<div iswin='1' class='dhtmlx_wins_body_outer'>"
				+ (_isIE
						? "<iframe frameborder='0' class='dhtmlx_wins_ie6_cover_fix' onload='this.contentWindow.document.body.style.overflow=\"hidden\";'></iframe>"
						: "")
		...
		win.skin = this.skin;
		win.dhxContGlobal.setContent(win.childNodes[0].childNodes[(_isIE
				? 5
				: 4)]);
	...
	this._engineRedrawWindowSize = function(win) {
		win.style.width = (String(win.w).search("%") == -1
				? win.w + "px"
				: win.w);
		win.style.height = (String(win.h).search("%") == -1
				? win.h + "px"
				: win.h);
	...
	this._engineCheckHeaderMouseDown = function(win, ev) {
		...
		if (y <= hh
				&& (obj == win.childNodes[0]
						...
						|| obj == win.childNodes[0].childNodes[(_isIE ? 3 : 2)] || obj == win.childNodes[0].childNodes[(_isIE
						? 4
						: 3)]))
			return true;
		return false
	...
	this._engineRedrawSkin = function() {
		...
		for (var a in this.wins) {
			if (this.skin == "dhx_skyblue") {
				this.wins[a].dhxContGlobal.obj._offsetTop = (this.wins[a]._noHeader
						? sk["noh_t"]
						: null);
				this.wins[a].dhxContGlobal.obj._offsetWidth = -10;
				this.wins[a].dhxContGlobal.obj._offsetHeight = (this.wins[a]._noHeader
						? sk["noh_h"]
						: -5);
	...
	this._engineSwitchWindowHeader = function(win, state) {
		...
		win.childNodes[0].childNodes[(_isIE ? 6 : 5)].style.display = (win._noHeader
				? ""
				: "none");
		win.childNodes[0].childNodes[(_isIE ? 1 : 0)].style.display = (win._noHeader
				? "none"
				: "");
		win.childNodes[0].childNodes[(_isIE ? 3 : 2)].style.display = (win._noHeader
				? "none"
				: "");
		win.childNodes[0].childNodes[(_isIE ? 4 : 3)].style.display = (win._noHeader
				? "none"
				: "");
		var sk = this._engineSkinParams[this.skin];
		if (win._noHeader) {
	...
	dhtmlx.extend_api("dhtmlXWindows", {
				...
				_viewport : function(data) {
					if (data.object) {
						...
						this.enableAutoViewport(false);
						this.setViewport(data.left, data.top, data.width,
								data.height, data.parent)
					}
				},
				_wins : function(arr) {
