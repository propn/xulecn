			...
			.subclass(
					'woo.menu',
					{
						options : {
							...
							showSpeed : 200,
							/**
							 * 显示菜单的方式：1.inner 在渲染的节点内部即点击的地方显示菜单
							 * 2.down在渲染的节点下方显示菜单
							 */
							showMenuType : 'inner',
							/**
						...
						_create : function() {
							...
							if ($.isArray(options.items)) {
								options.content = this
										.createContentByItems(options.items);
							}
							...
							if (options.type == 'click') {
								...
								el.mousedown(function(e) {
									...
									;
									return false;// 这里return
													// false后后面设置的累加及冒泡事件则不会触发
								});
						...
						showMenu : function(pos) {
							...
							el.addClass('fg-menu-open').addClass(
									options.callerOnState);
							container.hide().slideDown(options.showSpeed).find(
									'.fg-menu:eq(0)');
							menu.menuOpen = true;
						...
						create : function(pos) {
							...
							container.css({
								width : options.width
							}).appendTo('body').find('ul:first').not(
									'.fg-menu-breadcrumb').addClass('fg-menu');
							container.find('ul, li a')
									.addClass('ui-corner-all');// 加上制作圆角矩形的css,ie8无效

							// 分割线
							container.find("li[menuType$='line']").addClass(
									options.lineClass);

							if (container.find('ul').size() > 1) {
								menu.flyout(container);// 多级菜单处理
							} else {
								container.find('a[disabled!="true"]').click(
										function() {
											menu.chooseItem(this);
										});
							}
							;
							// 设置点击菜单事件，这里用live无法实现？container.find('.fg-menu
							// li')

							container.find(':data(item)').bind(
									"click",
									function(e) {
										var item = $(this).data("item");
										item.action.apply($(this), [ item, e,
												el ]);
									});
						...
						kill : function() {
							...
							container.find('li').removeClass(
									options.linkHoverSecondary).find('a')
							...
							caller
									...
									.each(
											function() {
												...
												var menuType = $(this).attr(
														'menuType');
												if (menuType == 'group') {
													menu
															.onePageGroupMenu($(this));
						...
						onePageGroupMenu : function(caller) {
							...
							caller.children("ul").children("li").each(
									function() {
										caller.append($(this));
						...
						setPosition : function(pos) {
							...
							var isHeperExist = (helper.size() == 0) ? false
									: true;
							if (!isHeperExist) {
							...
							if (options.positionOpts.directionV == 'up') {// 菜单往上展开
								...
								if (options.positionOpts.detectV
							...
							for ( var i = 0, l = items.length; i < l; i++) {
								...
								if (item.type == 'line') {// 分割线特殊处理
									...
									if ($.isFunction(item.renderer)) {
										text = item.renderer.apply(item,
												[ text ]);
									}
									var textClass = item.disabled ? "item-disabled"
											: options.itemTextClass;
									var li = $([
											"<li class='menu-link' menuType='"
													+ item.type + "'>",
									...
									if (items_) {
										...
										arguments.callee.apply(li, [ items_,
												++j, selfCmp ]);
									}
								}
							}
