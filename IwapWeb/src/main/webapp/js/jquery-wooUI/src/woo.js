...
(function($) {
			...
			.extend(
					$.woo,
					{
						...
						isPrimitive : function(v) {
							return $.woo.isString(v) || $.woo.isNumber(v)
									|| $.woo.isBoolean(v);
						},
						...
						/**
						 * 当传入的参数不是一个javascript
						 * undefined,null或者空的字符串，则返回true，否则返回false
						 * 
						 * @param {Mixed}
						 *            value
						 * @param {Boolean}
						 *            allowBlank (可选的) 是否把空字符串当做空
						 * @return {Boolean}
						 */
						...
						urlEncode : function(o, pre) {
							...
							for (key in o) {
								...
								$.woo.each(undef ? key : o[key], function(val,
										i) {
									buf.push('&', e(key), '=',
											(val != key || !undef) ? e(val)
													: '');
						 ...
						 *            string
						 * @param {Boolean}
						 *            overwrite (可选的) 同名参数用一个数组代替。默认为false。
						 * @return {Object}
						 */
						urlDecode : function(string, overwrite) {
							...
							for ( var i = 0, len = pairs.length; i < len; i++) {
								...
								name = decodeURIComponent(pairs[i]
										.slice(0, pos));
								value = decodeURIComponent(pairs[i]
										.slice(pos + 1));
								/*
								 * pair = pairs[i].split('='); name =
								 * decodeURIComponent(pair[0]); value =
						 ...
						 * <p>
						 * 注意，不要用于字符串，因为IE不支持"abc"[0]这样的写法。如果要将字符串转化为数组，可以使用"abc".match(/./g)
						 * ===> [a,b,c]
						 * </p>
						 * 
						 * @param {Iterable}
						...
						toArray : function() {
							// TODO
							// !$.support.htmlSerialize用于判断是ie浏览器，这个判断不合理，以后要改
							return !$.support.htmlSerialize ? function(a, i, j,
									res) {
								res = [];
						...
						/**
						 * 遍历一个数组或迭代一个对象的属性，并执行一个函数。 <b>Note</b>:
						 * 如果确定要遍历一个数组，最好使用each方法。
						 * 
						 * @param {Object/Array}
						 *            object
						 * @param {Function}
						 *            fn 如果函数返回false，则迭代将会停止。
						 */
						 ...
						 * 
						 * @param {Mixed}
						 *            value 应该是一个数字，但是任何类型都可以传入
						 * @param {Number}
						 *            defaultValue 原始值如果不是数字，则返回这个默认值
						...
						value : function(v, defaultValue, allowBlank) {
							return $.woo.isEmpty(v, allowBlank) ? defaultValue
									: v;
						},
						 ...
						 * 
						 * @param {Array|NodeList}
						 *            arr The Array from which to select the
						 *            minimum value.
						 * @param {Function}
						 *            comp (optional) a function to perform the
						 *            comparision which determines minimization.
						 ...
						 * 
						 * // 例子 2:
						 * TOONE.partition(TOONE.query('p'), function(val) {
						 ...
						 *            arr 需要区分的集合。
						 * @param {Function}
						 *            truth （可选的） 一个用来确定 true or false
						 *            的函数。如果省略该函数，则集合中的值本身必须可以进行true/false判断。
						 ...
						 * 
						 * <li>##.##</li>
						 * //如果该值为零，则不显示，返回一个空格的unicode编码 “&#160;”
						 * <li>##.#####</li>
						 * <li>##</li>
						 * 只显示四舍五入的整数
						 * <li>#0.##</li>
						 * //这个模板的意思是：如果该值为零，则强制显示，而不是转为空
						 * <li>###,###.##</li>
						 * <li>###,###</li>
						 * 分段的整数
						 * <li>###,##0.##</li>
	...
	/**
	 * 扩展选择器引擎的功能 $.extend($.expr[':'],{ inline: function(a) { return
	 * $(a).css('display') === 'inline'; } }); $(':inline'); // Selects ALL
	 * inline elements $('a:inline'); // Selects ALL inline anchors
	 */
	$
			.extend(
					$.expr[':'],
					{
						/**
						 * match:一个匹配数组 match[0]:data(key) match[1]:data
						 * match[2]:未知 match[3]:key
						 */
						...
						focusable : function(element) {
							var nodeName = element.nodeName.toLowerCase(), tabIndex = $
							...
							return (/input|select|textarea|button|object/
									...
									// be visible
									// the browser may report that the area is
									// hidden
