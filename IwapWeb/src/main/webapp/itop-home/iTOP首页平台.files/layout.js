...
(function($){
	...
	function setSize(container){//debugger;
	 ...
	 * 隐藏panel块
	 */
	function hidePanel(container, region){
		var panels = $.data(container, 'layout').panels;
		if(region == 'east'){
			panels.east.panel('close');
			panels.center.panel('resize', {
					width: panels.center.panel('options').width + panels.east.panel('options').width
				});
		}else if (region == 'west'){
			panels.west.panel('close');
			panels.center.panel('resize', {
				width: panels.center.panel('options').width + panels.west.panel('options').width,
				left: 0
			});
		}
	}
	 ...
	 * 显示panel块
	 */
	function showPanel(container, region){
		var panels = $.data(container, 'layout').panels;
		if(region == 'east'){
	...
	$.fn.layout = function(options, param){
		if (typeof options == 'string'){
			...
			case 'hide':
				return this.each(function(){
					hidePanel(this,param);
				});
			case 'show':
				return this.each(function(){
					showPanel(this,param);
				});
			case 'resize':
				return this.each(function(){
					setSize(this,param);
