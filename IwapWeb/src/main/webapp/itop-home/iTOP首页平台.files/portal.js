...
(function($) {
	...
	$.woo.component.subclass('woo.sortable', {
		options : {
			...
			// sortIndicator : $.woo.sortable.prototype._rearrange,modify by
			// chengl
			scope : "default",
		...
		_create : function() {
			...
			sortableItems.find(o.handleSelector).css({
						...
						if (!$(this).parent().hasClass('dragging')) {
							$(this).parent().css({
										width : $(this).parent().width() + 'px' //2010/11/20 amend by zhaob 里面的div宽度等于外部li的宽度就不会出现宽度变化
										//width : ''
									});
						} else {
		...
		_createItemMenu : function(menuItem) {
			var o = this.options, self = this;
			$('<a href="#" class="edit">菜单</a>').mousedown(function(e) {
						e.stopPropagation();
					}).menu({
				type : 'click',
				showMenuType : 'down',
				...
				items : [{
					text : "面板名称",
					action : function(item, e, el) {
						var title = $(el).parent().find('.tabs-title');
						var input = $("<input type='text' size='10' value='"
								+ title.text() + "'/>");
						title.html(input).focusout(function() {
									$(this).html($input.val());
								});
					}
				}, {
					text : "增加标签",
					action : function(item, e, el) {
						var ul = $(el).parents('ul.column');
						ul.sortable('addPortlet');
					}
				}, {
					text : "最大化",
					action : function(item, e, el) {
						...
						var ul = $(el).parents('ul.column');
						var oldUlWidth = ul.width();
						var oldUlHeight = ul.outerHeight();
						// 设置当前选择的列高度宽度并显示
						ul.width('100%');
						ul.height('100%');
						ul.children('li').width('97%');//2010/11/20 add by zhaob 最大化时内部li的宽度一起改变
						ul.show();
						// 隐藏该列的所有portlet
						ul.children('li').hide();
						// 点击的portlet显示
						var li = $(el).parents('li.widget');
						var oldLiHeight = li.height();
						var tab = li.find('.tabs-panels');

						li.find('iframe').height('95%');// 解决出现两条滚动条bug
						tab.height(oldUlHeight);
						li.show();
						// 点击的菜单文字及事件更改
						var menuText = this.find("[type='text']");
						menuText.text('最小化');
						this.unbind('click');
						this.bind('click', function() {
									...
									ul.children('li').show();
									// ul.width(oldWidth);
									ul.width('33.3%');// 这里用oldWidth在分辨率较低的时候出现换行，暂时设置死宽度百分比
									ul.children('li').width('');//2010/11/20 add by zhaob 最小化时内部li的宽度还原
									ul.height(oldUlHeight);
									menuText.text('最大化');
									self.unbind('click');
									self.bind('click', function() {
												item.action.apply(self, [item,
																e, el]);
											});
								});
					}
					...
					action : function(item, e, el) {
						if (confirm('确认关闭窗口？')) {
							$(el).parents(o.widgetSelector).animate({
										opacity : 0
									}, function() {
										$(this).wrap('<div/>').parent()
												.slideUp(function() {
															$(this).remove();
														});
									});
		...
		/**
		 * this.options.items 指所有portlet jquery对象集合数组 this.options.connectWith
		 * 指能被移动到的位置对象集合数组 此方法主要把组件通过data存储到每个li jquery对象上
		 */
		_refreshItems : function() {
			...
			// queries为一个多维数组[[$(this.options.items, this.element),
			...
			for (var i = queries.length - 1; i >= 0; i--) {
				queries[i][0].each(function() {
							// $(this)指每个li jquery对象
							$(this).data('sortable-item', queries[i][1]);
							items.push({
										item : $(this),
		...
		_mouseCapture : function(e, overrideHandle) {
			...
			if (this.options.handleSelector && !overrideHandle) {
				...
				$(this.options.handleSelector, currentItem).find("*").andSelf()
						.each(function() {
									if (this == e.target)
										validHandle = true;
								});
			...
			for (var i = 0; i < this.items.length; i++) {
				for (var j = 0; j < list.length; j++) {
					if (list[j] == this.items[i].item[0])
						this.items.splice(i, 1);// 删除从 i 处开始的1个元素
				};
			};
		},
		...
		_clear : function(e, noPropagation) {
			...
			if (!$.woo.contains(this.element[0], this.currentItem[0])) {
				this._propagate("remove", e, null, noPropagation);
				for (var i = this.containers.length - 1; i >= 0; i--) {
					if ($.woo.contains(this.containers[i].element[0],
							this.currentItem[0])) {
