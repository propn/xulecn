...
public class FrameServiceController {
	...
	public static void main(String[] args) {
		FrameServiceController sm = FrameServiceController.getInstance();
		try {
			TfmServicesVO xx = sm.getServiceByName("A");
			try {
				sm.doSomething("A");
	...
	public DynamicDict execQueryTypeService(DynamicDict dict, TfmServicesVO v)
		...
		if (dict.flag < 0) {
			if (GlobalNames.ERR_TO_DB) {
				long lErrSeq = 0;
				lErrSeq = LogHelper.writeLog(dict.m_ActionId, dict.flag,
						dict.msg, dict.exception == null ? "" : dict.exception);
			}
		}
		return dict;
	}
