...
public class ArcTpmBusiPriceParaDAO extends AbstractDAO {
	
	//查询SQL
	private String SQL_SELECT = "select price_id,owner_object_type,owner_object_id,para_name,seq,para_cname,para_type,data_type,para_value,para_value2,compute_rela_type,compute_method,compute_value,order_id,state,party_id,party_role_id,oper_date,para_value_cname,para_value2_cname from ARC_TPM_BUSI_PRICE_PARA where 1=1 ";
	
	//	统计总数SQL
	private String SQL_SELECT_COUNT = "select count(*) as col_counts from arc_tpm_busi_price_para where 1=1 ";

	//	insert SQl
	private String SQL_INSERT = "insert into ARC_TPM_BUSI_PRICE_PARA (price_id, owner_object_type, owner_object_id, para_name, seq, para_cname, para_type, data_type, para_value, para_value2, compute_rela_type, compute_method, compute_value, order_id, state, party_id, party_role_id, oper_date, para_value_cname, para_value2_cname) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

	//	普通update SQL
	private String SQL_UPDATE = "update ARC_TPM_BUSI_PRICE_PARA set price_id=?, owner_object_type=?, owner_object_id=?, para_name=?, seq=?, para_cname=?, para_type=?, data_type=?, para_value=?, para_value2=?, compute_rela_type=?, compute_method=?, compute_value=?, order_id=?, state=?, party_id=?, party_role_id=?, oper_date=?, para_value_cname=?, para_value2_cname=? where 1=1 ";
	
	//	普通delete SQL
	private String SQL_DELETE = "delete from ARC_TPM_BUSI_PRICE_PARA where 1=1 ";
	
		//	根据主键delete SQL
	private String SQL_DELETE_KEY = "delete from ARC_TPM_BUSI_PRICE_PARA where price_id=? and owner_object_type=? and owner_object_id=? and para_name=? and seq=?";
		
		//	根据主键update SQL
	private String SQL_UPDATE_KEY = "update ARC_TPM_BUSI_PRICE_PARA set price_id=?, owner_object_type=?, owner_object_id=?, para_name=?, seq=?, para_cname=?, para_type=?, data_type=?, para_value=?, para_value2=?, compute_rela_type=?, compute_method=?, compute_value=?, order_id=?, state=?, party_id=?, party_role_id=?, oper_date=?, para_value_cname=?, para_value2_cname=? where price_id=? and owner_object_type=? and owner_object_id=? and para_name=? and seq=?";
		
		//	根据主键查询SQL
	private String SQL_SELECT_KEY = "select price_id,owner_object_type,owner_object_id,para_name,seq,para_cname,para_type,data_type,para_value,para_value2,compute_rela_type,compute_method,compute_value,order_id,state,party_id,party_role_id,oper_date,para_value_cname,para_value2_cname from ARC_TPM_BUSI_PRICE_PARA where price_id=? and owner_object_type=? and owner_object_id=? and para_name=? and seq=? ";
	
	//	当前DAO 所属数据库name
	private String dbName = JNDINames.PM_DATASOURCE ;


	public ArcTpmBusiPriceParaDAO() {
