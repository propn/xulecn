...
public class ProductOffDAO extends AbstractDAO {
	
	//查询SQL
	private String SQL_SELECT = "select offer_id,seq,pricing_plan_id,offer_name,offer_comments,can_be_buy_alone,offer_code,state,eff_date,exp_date,priority_value,manage_grade,brand_id,offer_kind,comp_product_id,region_id,offer_provider_id,effective_type,effective_period_type,effective_period,relet_type,relet_period_type,relet_period,limit_check,send_bill,packet_type,oper_date,ord_action_type,ord_no,cancel_ord_no,event_seq,cancel_event_seq,price_comments,party_id,party_role_id,oper_region_id,serv_brand_id,sub_brand_id from PRODUCT_OFFER where 1=1 ";
	
	//	统计总数SQL
	private String SQL_SELECT_COUNT = "select count(*) as col_counts from product_offer where 1=1 ";

	//	insert SQl
	private String SQL_INSERT = "insert into PRODUCT_OFFER (offer_id, seq, pricing_plan_id, offer_name, offer_comments, can_be_buy_alone, offer_code, state, eff_date, exp_date, priority_value, manage_grade, brand_id, offer_kind, comp_product_id, region_id, offer_provider_id, effective_type, effective_period_type, effective_period, relet_type, relet_period_type, relet_period, limit_check, send_bill, packet_type, oper_date, ord_action_type, ord_no, cancel_ord_no, event_seq, cancel_event_seq, price_comments, party_id, party_role_id, oper_region_id, serv_brand_id, sub_brand_id) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

	//	普通update SQL
	private String SQL_UPDATE = "update PRODUCT_OFFER set offer_id=?, seq=?, pricing_plan_id=?, offer_name=?, offer_comments=?, can_be_buy_alone=?, offer_code=?, state=?, eff_date=?, exp_date=?, priority_value=?, manage_grade=?, brand_id=?, offer_kind=?, comp_product_id=?, region_id=?, offer_provider_id=?, effective_type=?, effective_period_type=?, effective_period=?, relet_type=?, relet_period_type=?, relet_period=?, limit_check=?, send_bill=?, packet_type=?, oper_date=?, ord_action_type=?, ord_no=?, cancel_ord_no=?, event_seq=?, cancel_event_seq=?, price_comments=?, party_id=?, party_role_id=?, oper_region_id=?, serv_brand_id=?, sub_brand_id=? where 1=1 ";
	
	//	普通delete SQL
	private String SQL_DELETE = "delete from PRODUCT_OFFER where 1=1 ";
	
		//	根据主键delete SQL
	private String SQL_DELETE_KEY = "delete from PRODUCT_OFFER where offer_id=? and seq=?";
		
		//	根据主键update SQL
	private String SQL_UPDATE_KEY = "update PRODUCT_OFFER set offer_id=?, seq=?, pricing_plan_id=?, offer_name=?, offer_comments=?, can_be_buy_alone=?, offer_code=?, state=?, eff_date=?, exp_date=?, priority_value=?, manage_grade=?, brand_id=?, offer_kind=?, comp_product_id=?, region_id=?, offer_provider_id=?, effective_type=?, effective_period_type=?, effective_period=?, relet_type=?, relet_period_type=?, relet_period=?, limit_check=?, send_bill=?, packet_type=?, oper_date=?, ord_action_type=?, ord_no=?, cancel_ord_no=?, event_seq=?, cancel_event_seq=?, price_comments=?, party_id=?, party_role_id=?, oper_region_id=?, serv_brand_id=?, sub_brand_id=? where offer_id=? and seq=?";
		
		//	根据主键查询SQL
	private String SQL_SELECT_KEY = "select offer_id,seq,pricing_plan_id,offer_name,offer_comments,can_be_buy_alone,offer_code,state,eff_date,exp_date,priority_value,manage_grade,brand_id,offer_kind,comp_product_id,region_id,offer_provider_id,effective_type,effective_period_type,effective_period,relet_type,relet_period_type,relet_period,limit_check,send_bill,packet_type,oper_date,ord_action_type,ord_no,cancel_ord_no,event_seq,cancel_event_seq,price_comments,party_id,party_role_id,oper_region_id,serv_brand_id,sub_brand_id from PRODUCT_OFFER where offer_id=? and seq=? ";
	
	//	当前DAO 所属数据库name
	private String dbName = JNDINames.PM_DATASOURCE ;


	public ProductOffDAO() {
