...
public class RcBaikaSimImportService extends HttpServlet {
        ...
        try {
            ...
            if (isMultipart) { // 是文件上传
                ...
                while (iter.hasNext()) {
                    ...
                    if (item.getFieldName().equals("simType")) {
                        pList.add(item.getString().trim());
                    ...
                    if (!item.isFormField()) {
                        ...
                        while ((str = read.readLine()) != null) {
                            ...
                            //ESN ICCID PIN1 PIN2 PUK1 PUK2 HRPD_SS HRPDUPP
                            if ((datas[0].length() > 30) ||
                                    !datas[1].matches("\\d{" +
                                        datas[1].length() + "}") ||
                                    (datas[1].length() > 50) ||
                                    (datas[2].length() > 30) ||
...
//
//                result = (String) bean.importRcBaikaSimInfo(map).get(0);
                
                List retList = (List)ServiceManager.callJavaBeanService("RcSimBO","importRcBaikaSimInfo" ,map);
