 ...
 * modifid: ...
 */
public class AttrConfigBO extends DictAction {
    public AttrConfigBO() {
    }
     ...
     *
     */
    public PageModel qryAttributeByPagination(DynamicDict dto)
      throws FrameException {
        ...
        String filterStr = DAOUtils.filterQureyLikeCond(attrName);
        String filterCode = DAOUtils.filterQureyLikeCond(attrCode);
        int count = 0;

        if (!"".equals(attrCode)) {
            cond.append(" attr_code='" + filterCode + "'");
            count++;
        }

        if (!"".equals(attrName)) {
            if (count > 0) {
                cond.append(" and attr_name like '%" + filterStr + "%'");
            } else {
                cond.append(" attr_name like '%" + filterStr + "%'");
            }
        }

        if ("".equals(attrCode) && "".equals(attrName)) {
            cond.append(" 1=1");
        }
        AttrDAO attrDAO = SrDAOFactory.getInstance().getAttrDAO();
		return PageHelper.popupPageModel(attrDAO, cond.toString(), pi,
				ps);
    }
