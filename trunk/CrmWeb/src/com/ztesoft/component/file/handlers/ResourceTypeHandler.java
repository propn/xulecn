...
public class ResourceTypeHandler {
	...
	private String name;
	private static Map types = new HashMap(4);
	private static Map paths = new HashMap(4);
...
//		paths.put(FILE, PropertiesLoader.getProperty("upload.resourceType.file.path"));
//		paths.put(IMAGE, PropertiesLoader.getProperty("upload.resourceType.image.path"));
//		paths.put(FLASH, PropertiesLoader.getProperty("upload.resourceType.flash.path"));
//		paths.put(MEDIA, PropertiesLoader.getProperty("upload.resourceType.media.path"));
	}
	...
	public static ResourceTypeHandler valueOf(final String name) throws IllegalArgumentException {
		...
		ResourceTypeHandler rt = (ResourceTypeHandler)types.get(name);
		if (rt == null)
			throw new IllegalArgumentException();
		return rt;
	}
