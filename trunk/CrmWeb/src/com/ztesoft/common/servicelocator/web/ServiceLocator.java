...
public class ServiceLocator {
	...
	public EJBHome getRemoteHome(String jndiHomeName, Class className,
			Object callerObj) throws ServiceLocatorException {
		EJBHome home = (EJBHome) cache.get(jndiHomeName);
		if (home == null) {
			try {
				Object objref = ic.lookup(jndiHomeName);
	...
	public QueueConnectionFactory getQueueConnectionFactory(
		...
		QueueConnectionFactory factory = (QueueConnectionFactory) cache
				.get(qConnFactoryName);
		if (factory == null) {
			try {
	...
	public TopicConnectionFactory getTopicConnectionFactory(
		...
		TopicConnectionFactory factory = (TopicConnectionFactory) cache
	...
	public UserTransaction getUserTransaction(String utName, Object callerObj)
			throws ServiceLocatorException {
		try {
			return (UserTransaction) ic.lookup(utName);
		} catch (Exception e) {
	...
	/*
	 * static Context getInitialContext() { Context ic = null;
	 * 
	 * try { Environment env = new Environment(); //
	 * env.setProviderUrl("t3://172.16.2.30:7001"); //测试
	 * //env.setProviderUrl("t3://172.16.6.48:8090"); //界面组用 /**
	 * env.setProviderUrl("t3://localhost:7001"); //江西环境
	 * //env.setProviderUrl("t3://134.224.4.69:8090"); //env.setProviderUrl(
	 * "t3://134.224.4.69:8090,localhost:8090,134.224.4.69:7001,localhost:7001"
	 * ); env.setSecurityPrincipal("weblogic");
	 * env.setSecurityCredentials("weblogic");
	 * 
	 * env.setProviderUrl(ConfigEnv.getProviderUrl());
	 * env.setSecurityPrincipal(ConfigEnv.getSecurityPrincipals());
	 * env.setSecurityCredentials(ConfigEnv.getSecurityCredentials()); ic =
	 * env.getInitialContext(); } catch (Exception e) {
