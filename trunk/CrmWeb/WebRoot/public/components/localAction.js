...
LocalAction.getLocalPathPrefix = function(){
	...
	var indexOfVsopWeb = href.lastIndexOf("VsopWeb");
	return href.slice(0, indexOfVsopWeb);	
}
...
LocalAction.getRemoteCallURL = function(localURL){
	return LocalAction.getLocalPathPrefix()+"VsopWeb/xml/"+localURL;	
}

LocalAction.getLocalMenuURL = function(Url){
	if(!Global.onServer && Url.indexOf("/VsopWeb/")!=-1){
...
LocalAction.getLocalTreeListStylePath = function(){
	return LocalAction.getLocalPathPrefix()+"VsopWeb/public/components/oaas-components/htc/treelist/";		
}
...
LocalAction.saveResponseData = function(data, localURL){
	if(Global.onServer && LocalAction.switch_recordData){
		var fso, f1, ts;
		var ForWriting = 2;
		var url = LocalAction.getLocalPathPrefix()+"VsopWeb/xml/"+localURL;
		fso = new ActiveXObject("Scripting.FileSystemObject");	
