...
Log.popupLogger = function(msg,level,obj) {
	...
	return;
       ...
       if (!obj._window || !obj._window.document) {
               obj._window = window.open("",'logger_popup_window','width=750,height=350,resizeable=1,scrollbars=1,status=0,toolbars=0');
               obj._window.opener.focus();
	       ...
	       if (obj._window.document && !obj._window.document.getElementById('loggerTable')) {
...
//定义通用的log对象 popupwindow logger
//var jsDebug = new Log(Log.DEBUG, Log.popupLogger);
var jsDebug = new Log(Log.DEBUG, Log.popupLogger);
